...
"use strict";var T=Object.defineProperty;var G=(r,e,t)=>e in r?T(r,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):r[e]=t;var a=(r,e,t)=>G(r,typeof e!="symbol"?e+"":e,t);const U=`<div role="dialog" class="relative z-10" aria-labelledby="modal-title" aria-modal="true">
  ...
  <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
    <div class="relative flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
      <div
        class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 w-full sm:w-[80%] md:w-[900px] min-h-screen sm:min-h-[70%] flex">
        ...
        <div class="main" id="main">
          <button class="top-0 right-0 text-lg fixed hover:bg-primary hover:text-white h-6 w-6 flex justify-center items-end rounded-full" id="close-modal">x</button>
...
`,tt=`<div id="trasferConfirmationProgress">
  <div class="box-container">
...
`;class et{constructor(e,t,A,n){a(this,"container");a(this,"contents");a(this,"closeModal");a(this,"email");a(this,"token");a(this,"_currentStep");a(this,"activeRef");a(this,"paymentResponse");a(this,"initialTime");a(this,"timeLeft");a(this,"timerId");a(this,"abortController",null);this.container=e,this.contents=document.querySelectorAll(".content"),this.closeModal=t,this.token=A,this.email=n,this._currentStep=1,this.renderTransferContent(),this.paymentResponse=null,this.activeRef="",this.initialTime=600,this.timeLeft=this.initialTime,this.timerId=null,this.attachInputListeners(),this.currentStep===1&&(this.abortController=new AbortController,this.createTransfer())}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderTransferContent(),e===1&&this.createTransfer(),e===5&&this.startTimer("check")}attachInputListeners(){const e=this.container.querySelector("#transfer-acn"),t=this.container.querySelector("#copy-acn"),A=this.container.querySelector("#acn-tooltip"),n=this.container.querySelector("#transfer-amount"),s=this.container.querySelector("#copy-amount"),i=this.container.querySelector("#amount-tooltip"),o=this.container.querySelector("#transfer-try-again"),c=this.container.querySelector("#transfer-sent"),h=this.container.querySelector(".transfer-button"),l=this.container.querySelector("#show-acn"),p=this.container.querySelector("#ctd-timer"),w=this.container.querySelector(".success-button"),S=this.container.querySelector("#confirm-one"),x=this.container.querySelector(".transfer-close");if(t&&e&&t.addEventListener("click",()=>{A&&this.copyToClipboard(e.innerHTML,A)}),s&&n&&s.addEventListener("click",()=>{i&&this.copyToClipboard(n.innerHTML.split(" ")[1],i)}),h&&h.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),o&&o.addEventListener("click",()=>{this.currentStep=1}),c&&c.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),l&&l.addEventListener("click",()=>{this.stopProgressBar(),this.currentStep=1}),this.currentStep===5){this.initialTime=300,this.timeLeft=this.initialTime,p&&(p.innerHTML=Q(this.initialTime));const b=3e4;this.abortController=new AbortController,setTimeout(()=>{this.verifyTransfer(this.token,b,()=>{this.currentStep=6},v=>{console.error("Polling failed:",v),d(v.message,"error")})},5e3),this.startTimer("check")}S&&S.addEventListener("click",()=>{this.currentStep=4}),w&&w.addEventListener("click",()=>this.closeModal()),x&&x.addEventListener("click",()=>this.closeModal())}async createTransfer(){requestAnimationFrame(()=>{var s;const e=this.container.querySelector("#loader"),t=this.container.querySelector("#details-content");e&&t&&(e.classList.remove("hidden"),(s=e.querySelector("img"))==null||s.setAttribute("src",y),t.classList.add("hidden"));const A={amount:14.99,channel:"bank_transfer",currency:"USD",customer:{email:this.email},reference:H()};z(this.token,A).then(i=>{if(i.status==="failed")i.providerMessage?d(i.providerMessage,"error"):d("Payment failed","error"),this.currentStep=2;else{e&&t&&(e.classList.add("hidden"),t.classList.remove("hidden")),this.activeRef=i.reference,this.paymentResponse=i,this.updateTransferDetailsContent();const o=3e4;setTimeout(()=>{this.verifyTransfer(this.token,o,()=>{this.currentStep=6},c=>{console.error("Polling failed:",c),d(c.message,"error")})},o)}}).catch(i=>{d(i.message,"error")})})}async verifyTransfer(e,t,A,n){let s=0,i=t,o=!0,c;this.abortController&&(c=this.abortController.signal);const h=async()=>{if(this.timeLeft<=0){o=!1,n(new Error("Max polling time reached")),this.currentStep=2;return}s+=1;try{const l=await q(e,this.activeRef,c);l.status==="failed"?o&&(i=t*(s+1),setTimeout(h,i)):(o=!1,A(l))}catch{o&&(i=t*(s+1),setTimeout(h,i))}};h()}getTransferStepContent(){switch(this.currentStep){case 1:return I;case 2:return X;case 3:return $;case 4:return _;case 5:return tt;case 6:return B;default:return I}}updateTransferDetailsContent(){var s;const e=this.container.querySelectorAll(".transfer-curr-amount"),t=this.container.querySelector("#transfer-bank"),A=this.container.querySelector("#transfer-acn"),n=this.container.querySelector("#time-left");this.paymentResponse&&(e.length&&e.forEach(i=>{var o,c;return i.innerHTML=`${(o=this.paymentResponse)==null?void 0:o.currency} ${(c=this.paymentResponse)==null?void 0:c.amount}`}),t&&this.paymentResponse.bankDetails&&(t.innerHTML=(s=this.paymentResponse.bankDetails)==null?void 0:s.bankName),A&&this.paymentResponse.bankDetails&&(A.innerHTML=this.paymentResponse.bankDetails.accountNumber),n&&(n.innerHTML=Q(this.initialTime)),this.startTimer("poll"))}renderTransferContent(){this.contents[1]&&(this.contents[1].innerHTML=this.getTransferStepContent(),this.attachInputListeners())}startTimer(e){const t=this.container.querySelector("#ctd-timer");this.stopTimer(),this.timerId=window.setInterval(()=>{this.timeLeft>0?(this.timeLeft-=1,e==="poll"?this.updateProgressTimerContent():t&&(t.innerHTML=Q(this.timeLeft),this.startTimer("check"))):(this.stopTimer(),e==="check"&&(this.currentStep=3))},1e3)}updateProgressTimerContent(){const t=2*Math.PI*40,n=(this.initialTime-this.timeLeft)/this.initialTime*100/100*t,s=this.container.querySelector(".circle-progress"),i=this.container.querySelector("#time-left");s&&(s.setAttribute("stroke-dasharray",`${t}`),s.setAttribute("stroke-dashoffset",`${n}`)),i&&(i.innerHTML=Q(this.timeLeft))}stopTimer(){this.timerId!==null&&(clearInterval(this.timerId),this.timerId=null)}stopProgressBar(){const e=this.container.querySelector(".progress-bar");e&&(e.style.animationPlayState="paused")}copyToClipboard(e,t){navigator.clipboard.writeText(e).then(()=>{t.innerHTML="Copied!"}).catch(()=>{t.innerHTML="Failed to copy"})}destroyTimer(){this.stopTimer()}stopPolling(){this.abortController&&this.abortController.abort()}}const f=`<div id="ussdBanks">
  <div class="box-container">
...
`,k=[{label:"Pay with Card",icon:`<svg width="23" height="19" viewBox="0 0 23 19" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`,key:"ussd"}],nt=[{name:"First City Monument Bank",code:"*329#"},{name:"First Bank",code:"*268#"}];class st{constructor(){a(this,"contents");a(this,"banks");a(this,"_currentStep");this.contents=document.querySelectorAll(".content"),this.banks=nt,this._currentStep=1,this.renderUssdContent()}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderUssdContent()}getUssdStepContent(){switch(this.currentStep){case 1:return f;case 2:return At;case 3:return L;case 4:return B;default:return f}}renderUssdContent(){this.contents[2]&&(this.contents[2].innerHTML=this.getUssdStepContent())}renderSearchOptions(e,t,A){A.innerHTML="",e.forEach(n=>{const s=document.createElement("div");s.classList.add("bank-option"),s.textContent=n.name,s.addEventListener("click",()=>{t.value=n.name,A.style.display="none"}),A.appendChild(s)}),A.style.display=e.length?"block":"none"}searchBanksOnInput(e,t){const A=e.value.toLowerCase(),n=this.banks.filter(s=>s.name.toLowerCase().includes(A));this.renderSearchOptions(n,e,t)}openSearchOnFocus(e,t){this.renderSearchOptions(this.banks,e,t)}closeSearchOptionsOutsideFocus(e,t,A){!t.contains(e.target)&&!A.contains(e.target)&&(A.style.display="none")}}class it{constructor(e,t){a(this,"modalContainer");a(this,"currentPaymentMethod");a(this,"currentStep");a(this,"container");a(this,"card");a(this,"transfer");a(this,"ussd");a(this,"merchantKey");a(this,"email");this.modalContainer=document.createElement("div"),this.modalContainer.innerHTML=U,document.body.appendChild(this.modalContainer),this.currentPaymentMethod=0,this.currentStep=1,this.container=document.querySelectorAll(".content")[0],this.merchantKey=e,this.email=t,this.updatePaymentMethodView(this.merchantKey,this.email),this.attachInputListeners()}attachInputListeners(){const e=this.modalContainer.querySelector(".success-button"),t=document.body.querySelector("#close-modal");e&&(this.cleanup(),e.addEventListener("click",this.closeModal)),t&&(this.cleanup(),t.addEventListener("click",()=>this.closeModal()))}displayTabLayout(){const e=document.querySelectorAll(".tab-button"),t=document.querySelector(".box-container-method");e&&t&&e.forEach(A=>{A.addEventListener("click",n=>{if(n.currentTarget instanceof HTMLElement&&n.currentTarget.dataset.tab){const s=parseInt(n.currentTarget.dataset.tab);this.updatePaymentMethodView(this.merchantKey,this.email),this.setCurrentPaymentMethod(s),this.cleanup(),this.renderPaymentMethodContent(),this.attachInputListeners()}})}),this.updatePaymentMethodView(this.merchantKey,this.email),this.renderPaymentMethodContent(),this.attachInputListeners(),this.setCurrentPaymentMethod(this.currentPaymentMethod)}updatePaymentMethodView(e,t){switch(this.currentPaymentMethod){case 0:return this.card=new M(this.modalContainer,()=>this.closeModal(),e,t);case 1:return this.transfer=new et(this.modalContainer,()=>this.closeModal(),e,t);case 2:return this.ussd=new st;default:return this.card=new M(this.modalContainer,()=>this.closeModal(),e,t)}}renderPaymentMethodContent(){return(()=>{switch(this.currentPaymentMethod){case 0:return this.card.renderCardContent();case 1:return this.transfer.renderTransferContent();case 2:return this.ussd.renderUssdContent();default:return this.card.renderCardContent()}})()}setCurrentPaymentMethod(e){this.currentPaymentMethod=e;const t=document.querySelectorAll(".tab-button"),A=document.querySelectorAll(".content");t.forEach(n=>n.classList.remove("active")),A.forEach(n=>n.classList.remove("active")),t[e]&&t[e].classList.add("active"),A[e]&&A[e].classList.add("active")}cleanup(){this.transfer&&this.transfer.destroyTimer()}closeModal(){this.modalContainer&&this.modalContainer.parentNode&&document.body.removeChild(this.modalContainer)}setup(){const e=document.getElementById("merchantLogo"),t=document.getElementById("merchant-email");e&&e.setAttribute("src",J),t&&(t.innerHTML=this.email);let A=document.querySelector(".payment-options");A&&(A.innerHTML=`<div> 
      ${k.map((o,c)=>`
            <div class="tab">   
              <button class="tab-button group" data-tab="${c}"> 
              <div class="min-w-8">
                ${o.icon}
                </div>
                <span>${o.label}</span>
          ...
          `).join("")}
      </div>`);const n=document.querySelectorAll(".tab-button");k.forEach((o,c)=>{const h=n[c];if(h){const l=h.querySelector(".tab-button-icon"),p=h.querySelector(".tab-button-label");l&&(l.innerHTML=o.icon),p&&(p.textContent=o.label),h.dataset.tab=c.toString(),h.addEventListener("click",()=>{this.setCurrentPaymentMethod(c)})}}),this.displayTabLayout();const s=this.modalContainer.querySelector("#paymentWarning"),i=s==null?void 0:s.querySelector("#payment-warning-text");i&&this.displayPaymentWarningText(this.currentPaymentMethod,i)}displayPaymentWarningText(e,t){const A=()=>{switch(e){case 0:return"Incorrect otp. please retry with the correct otp";case 1:return"Account expired";case 2:return"Please dial the ussd shortcode";default:return"Error"}};t&&(t.innerHTML=A())}}module.exports=it;
