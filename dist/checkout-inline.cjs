...
"use strict";var T=Object.defineProperty;var G=(r,A,t)=>A in r?T(r,A,{enumerable:!0,configurable:!0,writable:!0,value:t}):r[A]=t;var a=(r,A,t)=>G(r,typeof A!="symbol"?A+"":A,t);const U=`<div role="dialog" class="relative z-10" aria-labelledby="modal-title" aria-modal="true">
...
`;class At{constructor(A,t,e,n){a(this,"container");a(this,"contents");a(this,"closeModal");a(this,"email");a(this,"token");a(this,"_currentStep");a(this,"activeRef");a(this,"paymentResponse");a(this,"initialTime");a(this,"timeLeft");a(this,"timerId");a(this,"abortController",null);this.container=A,this.contents=document.querySelectorAll(".content"),this.closeModal=t,this.token=e,this.email=n,this._currentStep=1,this.renderTransferContent(),this.paymentResponse=null,this.activeRef="",this.initialTime=600,this.timeLeft=this.initialTime,this.timerId=null,this.attachInputListeners(),this.currentStep===1&&(this.abortController=new AbortController,this.createTransfer())}get currentStep(){return this._currentStep}set currentStep(A){this._currentStep=A,this.renderTransferContent(),A===1&&this.createTransfer(),A===5&&this.startTimer("check")}attachInputListeners(){const A=this.container.querySelector("#transfer-acn"),t=this.container.querySelector("#copy-acn"),e=this.container.querySelector("#acn-tooltip"),n=this.container.querySelector("#transfer-amount"),i=this.container.querySelector("#copy-amount"),s=this.container.querySelector("#amount-tooltip"),o=this.container.querySelector("#transfer-try-again"),c=this.container.querySelector("#transfer-sent"),C=this.container.querySelector(".transfer-button"),l=this.container.querySelector("#show-acn"),g=this.container.querySelector("#ctd-timer"),p=this.container.querySelector(".success-button"),S=this.container.querySelector("#confirm-one"),x=this.container.querySelector(".transfer-close");if(t&&A&&t.addEventListener("click",()=>{e&&this.copyToClipboard(A.innerHTML,e)}),i&&n&&i.addEventListener("click",()=>{s&&this.copyToClipboard(n.innerHTML.split(" ")[1],s)}),C&&C.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),o&&o.addEventListener("click",()=>{this.currentStep=1}),c&&c.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),l&&l.addEventListener("click",()=>{this.stopProgressBar(),this.currentStep=1}),this.currentStep===5){this.initialTime=300,this.timeLeft=this.initialTime,g&&(g.innerHTML=Q(this.initialTime));const b=3e4;this.abortController=new AbortController,setTimeout(()=>{this.verifyTransfer(this.token,b,()=>{this.currentStep=6},v=>{console.error("Polling failed:",v),d(v.message,"error")})},5e3),this.startTimer("check")}S&&S.addEventListener("click",()=>{this.currentStep=4}),p&&p.addEventListener("click",()=>this.closeModal()),x&&x.addEventListener("click",()=>this.closeModal())}async createTransfer(){requestAnimationFrame(()=>{var i;const A=this.container.querySelector("#loader"),t=this.container.querySelector("#details-content");A&&t&&(A.classList.remove("hidden"),(i=A.querySelector("img"))==null||i.setAttribute("src",y),t.classList.add("hidden"));const e={amount:14.99,channel:"bank_transfer",currency:"USD",customer:{email:this.email},reference:H()};z(this.token,e).then(s=>{if(s.status==="failed")s.providerMessage?d(s.providerMessage,"error"):d("Payment failed","error"),this.currentStep=2;else{A&&t&&(A.classList.add("hidden"),t.classList.remove("hidden")),this.activeRef=s.reference,this.paymentResponse=s,this.updateTransferDetailsContent();const o=3e4;setTimeout(()=>{this.verifyTransfer(this.token,o,()=>{this.currentStep=6},c=>{console.error("Polling failed:",c),d(c.message,"error")})},o)}}).catch(s=>{d(s.message,"error")})})}async verifyTransfer(A,t,e,n){let i=0,s=t,o=!0,c;this.abortController&&(c=this.abortController.signal);const C=async()=>{if(this.timeLeft<=0){o=!1,n(new Error("Max polling time reached")),this.currentStep=2;return}i+=1;try{const l=await q(A,this.activeRef,c);l.status==="failed"?o&&(s=t*(i+1),setTimeout(C,s)):(o=!1,e(l))}catch{o&&(s=t*(i+1),setTimeout(C,s))}};C()}getTransferStepContent(){switch(this.currentStep){case 1:return I;case 2:return X;case 3:return $;case 4:return _;case 5:return tt;case 6:return B;default:return I}}updateTransferDetailsContent(){var i;const A=this.container.querySelectorAll(".transfer-curr-amount"),t=this.container.querySelector("#transfer-bank"),e=this.container.querySelector("#transfer-acn"),n=this.container.querySelector("#time-left");this.paymentResponse&&(A.length&&A.forEach(s=>{var o,c;return s.innerHTML=`${(o=this.paymentResponse)==null?void 0:o.currency} ${(c=this.paymentResponse)==null?void 0:c.amount}`}),t&&this.paymentResponse.bankDetails&&(t.innerHTML=(i=this.paymentResponse.bankDetails)==null?void 0:i.bankName),e&&this.paymentResponse.bankDetails&&(e.innerHTML=this.paymentResponse.bankDetails.accountNumber),n&&(n.innerHTML=Q(this.initialTime)),this.startTimer("poll"))}renderTransferContent(){this.contents[1]&&(this.contents[1].innerHTML=this.getTransferStepContent(),this.attachInputListeners())}startTimer(A){const t=this.container.querySelector("#ctd-timer");this.stopTimer(),this.timerId=window.setInterval(()=>{this.timeLeft>0?(this.timeLeft-=1,A==="poll"?this.updateProgressTimerContent():t&&(t.innerHTML=Q(this.timeLeft),this.startTimer("check"))):(this.stopTimer(),A==="check"&&(this.currentStep=3))},1e3)}updateProgressTimerContent(){const t=2*Math.PI*40,n=(this.initialTime-this.timeLeft)/this.initialTime*100/100*t,i=this.container.querySelector(".circle-progress"),s=this.container.querySelector("#time-left");i&&(i.setAttribute("stroke-dasharray",`${t}`),i.setAttribute("stroke-dashoffset",`${n}`)),s&&(s.innerHTML=Q(this.timeLeft))}stopTimer(){this.timerId!==null&&(clearInterval(this.timerId),this.timerId=null)}stopProgressBar(){const A=this.container.querySelector(".progress-bar");A&&(A.style.animationPlayState="paused")}copyToClipboard(A,t){navigator.clipboard.writeText(A).then(()=>{t.innerHTML="Copied!"}).catch(()=>{t.innerHTML="Failed to copy"})}destroyTimer(){this.stopTimer()}stopPolling(){this.abortController&&this.abortController.abort()}}const f=`<div id="ussdBanks">
  <div class="box-container">
