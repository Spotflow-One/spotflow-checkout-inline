...
(function(w,m){typeof exports=="object"&&typeof module<"u"?module.exports=m():typeof define=="function"&&define.amd?define(m):(w=typeof globalThis<"u"?globalThis:w||self,w.SpotflowCheckout=m())})(this,function(){"use strict";var r1=Object.defineProperty;var a1=(w,m,g)=>m in w?r1(w,m,{enumerable:!0,configurable:!0,writable:!0,value:g}):w[m]=g;var a=(w,m,g)=>a1(w,typeof m!="symbol"?m+"":m,g);const w=`<div role="dialog" class="relative z-10" aria-labelledby="modal-title" aria-modal="true">
...
`,key:"ussd"}],j=[{name:"First City Monument Bank",code:"*329#"},{name:"First Bank",code:"*268#"}],B=[{name:"MasterCard",icon:`<svg width="24" height="14" viewBox="0 0 24 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`}];class k{constructor(e,t,n,s,A){a(this,"cardDetailsValues");a(this,"cardPin");a(this,"cardOtp");a(this,"contents");a(this,"container");a(this,"closeModal");a(this,"email");a(this,"token");a(this,"amount");a(this,"_currentStep");a(this,"activeRef");a(this,"creditCardTypes");this.container=e,this.closeModal=t,this.token=n,this.email=s,this.amount=A,this._currentStep=1,this.cardDetailsValues={number:"",expiry:"",cvv:""},this.cardPin="",this.cardOtp="",this.activeRef="",this.contents=document.querySelectorAll(".content"),this.creditCardTypes=[...B],this.renderCardContent(),this.currentStep===1&&this.displayCardTypes(),this.attachInputListeners()}displayCardTypes(){requestAnimationFrame(()=>{const e=this.container.querySelector("#card-types");this.creditCardTypes.length===1&&this.creditCardTypes[0].name==="Unknown"?B.forEach(t=>{const n=document.createElement("div");n.classList.add("card-type"),n.innerHTML=t.icon,e.appendChild(n)}):(e.innerHTML="",this.creditCardTypes.forEach(t=>{const n=document.createElement("div");n.classList.add("card-type"),n.innerHTML=t.icon,e.appendChild(n)}))})}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderCardContent(),e===1&&this.displayCardTypes()}attachInputListeners(){const e=this.container.querySelector('input[name="number"]'),t=this.container.querySelector('input[name="expiry"]'),n=this.container.querySelector('input[name="cvv"]'),s=this.container.querySelector(".details-form-button"),A=document.querySelector("#checkoutcard"),i=Array.from(this.container.querySelectorAll(".pin-input")),c=this.container.querySelector(".otp-input"),o=this.container.querySelector(".otp-button"),C=this.container.querySelector(".success-button");e&&e.addEventListener("input",l=>this.handleInputChange(l,s)),t&&t.addEventListener("input",l=>this.handleInputChange(l,s)),n&&n.addEventListener("input",l=>this.handleInputChange(l,s)),A&&A.addEventListener("submit",l=>this.handleSubmit(l)),i.forEach((l,d)=>{l.addEventListener("input",p=>this.handlePinInputChange(p,d,i)),l.addEventListener("paste",p=>this.handlePinPaste(p,i))}),c&&c.addEventListener("input",l=>this.handleOtpInput(l,o)),o&&o.addEventListener("click",l=>this.submitOtp(l)),C&&C.addEventListener("click",()=>this.closeModal())}handleInputChange(e,t){const n=e.target,{name:s,value:A}=n;let i=A;s==="number"?(i=q(A),this.filterCreditCardType(A)):s==="expiry"?i=z(A):s==="cvv"&&(i=Y(A)),this.cardDetailsValues={...this.cardDetailsValues,[s]:i},n.value=i,this.cardDetailsValues.number&&this.cardDetailsValues.expiry&&this.cardDetailsValues.cvv&&t&&t.removeAttribute("disabled")}handlePinRequest(){const e=this.container.querySelector("#loader"),t=this.container.querySelector("#pin-content");if(this.cardPin.length===4){const n={authorization:{pin:this.cardPin},reference:this.activeRef};setTimeout(()=>{var A;e&&t&&(e.classList.remove("hidden"),(A=e.querySelector("img"))==null||A.setAttribute("src",f),t.classList.add("hidden")),K(this.token,n).then(i=>{i.status==="failed"?i.providerMessage?h(i.providerMessage,"error"):h("Payment failed","error"):(h("Payment Authorized!","success"),this.currentStep=3)}).catch(i=>{h(i.message,"error")}).finally(()=>{e&&t&&(e.classList.add("hidden"),t.classList.remove("hidden"))})},500)}}handlePinInputChange(e,t,n){this.cardPin="",e.target.value?t<n.length-1&&n[t+1].focus():t>0&&n[t-1].focus(),this.cardPin=n.map(i=>i.value).join(""),this.handlePinRequest()}handlePinPaste(e,t){var A;e.preventDefault(),this.cardPin="";const s=(((A=e.clipboardData)==null?void 0:A.getData("text").trim())||"").split("");t.forEach((i,c)=>{i.value=s[c]||""});for(let i=0;i<t.length;i++)if(!t[i].value){t[i].focus();break}this.cardPin=t.map(i=>i.value).join(""),this.handlePinRequest()}handleOtpInput(e,t){const n=e.target,{value:s}=n;this.cardOtp="",this.cardOtp=s,this.cardOtp.length>3&&t&&t.removeAttribute("disabled")}submitOtp(e){var i;e.preventDefault();const t=this.container.querySelector("#loader"),n=this.container.querySelector("#otp-content"),s={authorization:{otp:this.cardPin},reference:this.activeRef};t&&n&&(t.classList.remove("hidden"),(i=t.querySelector("img"))==null||i.setAttribute("src",f),n.classList.add("hidden")),O(this.token,s).then(c=>{c.status==="failed"?c.providerMessage?h(c.providerMessage,"error"):h("Payment failed","error"):c.status==="successful"&&(this.currentStep=5)}).catch(c=>{h(c.message,"error")}).finally(()=>{t&&n&&(t.classList.add("hidden"),n.classList.remove("hidden"))})}async handleSubmit(e){var i,c;e.preventDefault();const t=this.container.querySelector(".spinner"),n=this.container.querySelector("#details-form-button-text"),s={amount:this.amount,channel:"card",currency:"USD",customer:{email:this.email},reference:b(),card:{pan:V(this.cardDetailsValues.number),cvv:this.cardDetailsValues.cvv,expiryMonth:(i=this.cardDetailsValues)==null?void 0:i.expiry.split("/")[0].trim(),expiryYear:(c=this.cardDetailsValues)==null?void 0:c.expiry.split("/")[1].trim()}};t&&n&&(t.classList.remove("hidden"),n.classList.add("hidden")),J(this.token,s).then(o=>{o.status==="failed"?o.providerMessage?h(o.providerMessage,"error"):h("Payment failed","error"):o.status==="pending_authorization"&&(h("Payment Created!","success"),this.activeRef=o.reference,this.currentStep=2)}).catch(o=>{h(o.message,"error")}).finally(()=>{t&&n&&(t.classList.add("hidden"),n.classList.remove("hidden"))})}getCardStepContent(){switch(this.currentStep){case 1:return g;case 2:return U;case 3:return D;case 4:return R;case 5:return x;default:return g}}filterCreditCardType(e){e?(this.creditCardTypes=B.filter(t=>t.name===P(e)),this.creditCardTypes.length>=1?this.displayCardTypes():(this.creditCardTypes=[...B],this.displayCardTypes())):(this.creditCardTypes=[...B],this.displayCardTypes())}renderCardContent(){this.contents[0]&&(this.contents[0].innerHTML=this.getCardStepContent(),this.attachInputListeners())}}const T=`<div>
  <div class="box-container">
...
</div>`,$=`<div id="trasferConfirmationOne">
    <div class="box-container">
...
`,t1=`<div id="trasferConfirmationProgress">
  <div class="box-container">
...
`;class e1{constructor(e,t,n,s,A){a(this,"container");a(this,"contents");a(this,"closeModal");a(this,"email");a(this,"token");a(this,"amount");a(this,"_currentStep");a(this,"activeRef");a(this,"paymentResponse");a(this,"initialTime");a(this,"timeLeft");a(this,"timerId");a(this,"abortController",null);this.container=e,this.contents=document.querySelectorAll(".content"),this.closeModal=t,this.token=n,this.email=s,this.amount=A,this._currentStep=1,this.renderTransferContent(),this.paymentResponse=null,this.activeRef="",this.initialTime=600,this.timeLeft=this.initialTime,this.timerId=null,this.attachInputListeners(),this.currentStep===1&&(this.abortController=new AbortController,this.createTransfer())}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderTransferContent(),e===1&&this.createTransfer(),e===5&&this.startTimer("check")}attachInputListeners(){const e=this.container.querySelector("#transfer-acn"),t=this.container.querySelector("#copy-acn"),n=this.container.querySelector("#acn-tooltip"),s=this.container.querySelector("#transfer-amount"),A=this.container.querySelector("#copy-amount"),i=this.container.querySelector("#amount-tooltip"),c=this.container.querySelector("#transfer-try-again"),o=this.container.querySelector("#transfer-sent"),C=this.container.querySelector(".transfer-button"),l=this.container.querySelector("#show-acn"),d=this.container.querySelector("#ctd-timer"),p=this.container.querySelector(".success-button"),H=this.container.querySelector("#confirm-one"),S=this.container.querySelector(".transfer-close");if(t&&e&&t.addEventListener("click",()=>{n&&this.copyToClipboard(e.innerHTML,n)}),A&&s&&A.addEventListener("click",()=>{i&&this.copyToClipboard(s.innerHTML.split(" ")[1],i)}),C&&C.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),c&&c.addEventListener("click",()=>{this.currentStep=1}),o&&o.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),l&&l.addEventListener("click",()=>{this.stopProgressBar(),this.currentStep=1}),this.currentStep===5){this.initialTime=300,this.timeLeft=this.initialTime,d&&(d.innerHTML=M(this.initialTime));const i1=3e4;this.abortController=new AbortController,setTimeout(()=>{this.verifyTransfer(this.token,i1,()=>{this.currentStep=6},G=>{console.error("Polling failed:",G),h(G.message,"error")})},5e3),this.startTimer("check")}H&&H.addEventListener("click",()=>{this.currentStep=4}),p&&p.addEventListener("click",()=>this.closeModal()),S&&S.addEventListener("click",()=>this.closeModal())}async createTransfer(){requestAnimationFrame(()=>{var A;const e=this.container.querySelector("#loader"),t=this.container.querySelector("#details-content");e&&t&&(e.classList.remove("hidden"),(A=e.querySelector("img"))==null||A.setAttribute("src",f),t.classList.add("hidden"));const n={amount:this.amount,channel:"bank_transfer",currency:"USD",customer:{email:this.email},reference:b()};Z(this.token,n).then(i=>{if(i.status==="failed")i.providerMessage?h(i.providerMessage,"error"):h("Payment failed","error"),this.currentStep=2;else{e&&t&&(e.classList.add("hidden"),t.classList.remove("hidden")),this.activeRef=i.reference,this.paymentResponse=i,this.updateTransferDetailsContent();const c=3e4;setTimeout(()=>{this.verifyTransfer(this.token,c,()=>{this.currentStep=6},o=>{console.error("Polling failed:",o),h(o.message,"error")})},c)}}).catch(i=>{h(i.message,"error")})})}async verifyTransfer(e,t,n,s){let A=0,i=t,c=!0,o;this.abortController&&(o=this.abortController.signal);const C=async()=>{if(this.timeLeft<=0){c=!1,s(new Error("Max polling time reached")),this.currentStep=2;return}A+=1;try{const l=await N(e,this.activeRef,o);l.status==="failed"?c&&(i=t*(A+1),setTimeout(C,i)):(c=!1,n(l))}catch{c&&(i=t*(A+1),setTimeout(C,i))}};C()}getTransferStepContent(){switch(this.currentStep){case 1:return T;case 2:return X;case 3:return $;case 4:return _;case 5:return t1;case 6:return x;default:return T}}updateTransferDetailsContent(){var A;const e=this.container.querySelectorAll(".transfer-curr-amount"),t=this.container.querySelector("#transfer-bank"),n=this.container.querySelector("#transfer-acn"),s=this.container.querySelector("#time-left");this.paymentResponse&&(e.length&&e.forEach(i=>{var c,o;return i.innerHTML=`${(c=this.paymentResponse)==null?void 0:c.currency} ${(o=this.paymentResponse)==null?void 0:o.amount}`}),t&&this.paymentResponse.bankDetails&&(t.innerHTML=(A=this.paymentResponse.bankDetails)==null?void 0:A.bankName),n&&this.paymentResponse.bankDetails&&(n.innerHTML=this.paymentResponse.bankDetails.accountNumber),s&&(s.innerHTML=M(this.initialTime)),this.startTimer("poll"))}renderTransferContent(){this.contents[1]&&(this.contents[1].innerHTML=this.getTransferStepContent(),this.attachInputListeners())}startTimer(e){const t=this.container.querySelector("#ctd-timer");this.stopTimer(),this.timerId=window.setInterval(()=>{this.timeLeft>0?(this.timeLeft-=1,e==="poll"?this.updateProgressTimerContent():t&&(t.innerHTML=M(this.timeLeft),this.startTimer("check"))):(this.stopTimer(),e==="check"&&(this.currentStep=3))},1e3)}updateProgressTimerContent(){const t=2*Math.PI*40,s=(this.initialTime-this.timeLeft)/this.initialTime*100/100*t,A=this.container.querySelector(".circle-progress"),i=this.container.querySelector("#time-left");A&&(A.setAttribute("stroke-dasharray",`${t}`),A.setAttribute("stroke-dashoffset",`${s}`)),i&&(i.innerHTML=M(this.timeLeft))}stopTimer(){this.timerId!==null&&(clearInterval(this.timerId),this.timerId=null)}stopProgressBar(){const e=this.container.querySelector(".progress-bar");e&&(e.style.animationPlayState="paused")}copyToClipboard(e,t){navigator.clipboard.writeText(e).then(()=>{t.innerHTML="Copied!"}).catch(()=>{t.innerHTML="Failed to copy"})}destroyTimer(){this.stopTimer()}stopPolling(){this.abortController&&this.abortController.abort()}}const F=`<div id="ussdBanks">
  <div class="box-container">
...
`;class A1{constructor(){a(this,"contents");a(this,"banks");a(this,"_currentStep");this.contents=document.querySelectorAll(".content"),this.banks=j,this._currentStep=1,this.renderUssdContent()}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderUssdContent()}getUssdStepContent(){switch(this.currentStep){case 1:return F;case 2:return n1;case 3:return R;case 4:return x;default:return F}}renderUssdContent(){this.contents[2]&&(this.contents[2].innerHTML=this.getUssdStepContent())}renderSearchOptions(e,t,n){n.innerHTML="",e.forEach(s=>{const A=document.createElement("div");A.classList.add("bank-option"),A.textContent=s.name,A.addEventListener("click",()=>{t.value=s.name,n.style.display="none"}),n.appendChild(A)}),n.style.display=e.length?"block":"none"}searchBanksOnInput(e,t){const n=e.value.toLowerCase(),s=this.banks.filter(A=>A.name.toLowerCase().includes(n));this.renderSearchOptions(s,e,t)}openSearchOnFocus(e,t){this.renderSearchOptions(this.banks,e,t)}closeSearchOptionsOutsideFocus(e,t,n){!t.contains(e.target)&&!n.contains(e.target)&&(n.style.display="none")}}class s1{constructor(e,t,n){a(this,"modalContainer");a(this,"currentPaymentMethod");a(this,"currentStep");a(this,"container");a(this,"card");a(this,"transfer");a(this,"ussd");a(this,"merchantKey");a(this,"email");a(this,"amount");a(this,"isMobile",!1);if(this.modalContainer=document.createElement("div"),this.modalContainer.innerHTML=w,document.body.appendChild(this.modalContainer),this.isMobile=window.matchMedia("(max-width: 767px)").matches,this.currentPaymentMethod=this.isMobile?3:0,this.currentStep=1,this.container=document.querySelectorAll(".content")[0],this.merchantKey=e,this.email=t,this.amount=n,this.attachInputListeners(),e===""||t==="")throw this.closeModal(),new Error("Merchant key or email is required")}attachInputListeners(){const e=document.body.querySelector("#change-method"),t=document.body.querySelectorAll(".close-btn"),n=document.querySelector("#mob-container"),s=document.querySelector("#mob-tab-co"),A=document.querySelector("#mob-tab-c");e&&(this.cleanup(),e.addEventListener("click",()=>{this.currentPaymentMethod=3,this.attachInputListeners(),n&&(n.innerHTML=this.mobileContainerContent()),this.setup(),s&&A&&A.parentNode&&s.removeChild(A)})),t.length>0&&t.forEach(i=>{i.addEventListener("click",()=>{this.cleanup(),this.closeModal()})})}displayTabLayout(){const e=document.querySelectorAll(".tab-button"),t=document.querySelectorAll(".mob-tab-button"),n=document.querySelector(".box-container-method"),s=document.querySelector("#mob-container"),A=document.querySelector("#inner-mob-container"),i=document.querySelector("#mob-tab-co");this.isMobile?t&&n&&s&&A&&t.forEach(c=>{c.addEventListener("click",o=>{if(o.currentTarget instanceof HTMLElement&&o.currentTarget.dataset.tab){const C=parseInt(o.currentTarget.dataset.tab);this.updatePaymentMethodView(this.merchantKey,this.email,this.amount),this.setCurrentPaymentMethod(C),s.removeChild(A),this.cleanup(),Q.forEach((l,d)=>{this.currentPaymentMethod===d&&i&&(i.innerHTML=`
                          <div id="mob-tab-c" class="w-full pt-2 px-5 md:hidden">
                            <div>
                              <button class="pt-3 px-2 grow align-center text-grey-600 tex-sm font-semibold" data-tab="${d}">
                      ...
                      `)}),this.renderPaymentMethodContent(),this.attachInputListeners()}})}):e&&n&&e.forEach(c=>{c.addEventListener("click",o=>{if(o.currentTarget instanceof HTMLElement&&o.currentTarget.dataset.tab){const C=parseInt(o.currentTarget.dataset.tab);this.updatePaymentMethodView(this.merchantKey,this.email,this.amount),this.setCurrentPaymentMethod(C),this.cleanup(),this.renderPaymentMethodContent(),this.attachInputListeners()}})}),this.updatePaymentMethodView(this.merchantKey,this.email,this.amount),this.renderPaymentMethodContent(),this.attachInputListeners(),this.setCurrentPaymentMethod(this.currentPaymentMethod)}updatePaymentMethodView(e,t,n){switch(this.currentPaymentMethod){case 0:return this.card=new k(this.modalContainer,()=>this.closeModal(),e,t,n);case 1:return this.transfer=new e1(this.modalContainer,()=>this.closeModal(),e,t,n);case 2:return this.ussd=new A1;default:return this.card=new k(this.modalContainer,()=>this.closeModal(),e,t,n)}}renderPaymentMethodContent(){return(()=>{switch(this.currentPaymentMethod){case 0:return this.card.renderCardContent();case 1:return this.transfer.renderTransferContent();case 2:return this.ussd.renderUssdContent();case 3:return"";default:return this.card.renderCardContent()}})()}setCurrentPaymentMethod(e){this.currentPaymentMethod=e;const t=document.querySelectorAll(".tab-button");let n=document.querySelectorAll(".content");const s=document.getElementById("mob-action-btns");t.forEach(A=>A.classList.remove("active")),n.forEach(A=>A.classList.remove("active")),t[e]&&t[e].classList.add("active"),n[e]&&n[e].classList.add("active"),e!==3&&s&&(s.classList.remove("hidden"),s.classList.add("md:hidden"))}cleanup(){this.transfer&&this.transfer.destroyTimer()}closeModal(){this.modalContainer&&this.modalContainer.parentNode&&document.body.removeChild(this.modalContainer)}setup(){const e=document.getElementById("merchantLogo"),t=document.getElementById("merchantLogoMob"),n=document.getElementById("mob-action-btns"),s=document.getElementById("merchant-email"),A=this.modalContainer.querySelector("#paymentWarning"),i=A==null?void 0:A.querySelector("#payment-warning-text"),c=document.querySelector("#mob-container");if(s&&(s.innerHTML=this.email),this.isMobile){t&&t.setAttribute("src",m),c&&(c.innerHTML=this.mobileContainerContent());let o=document.querySelector(".mob-options");o&&this.viewMobileOptions(o);const C=document.querySelectorAll(".mob-tab-button");Q.forEach((l,d)=>{const p=C[d];p&&(p.dataset.tab=d.toString(),p.addEventListener("click",()=>{this.setCurrentPaymentMethod(d)}))}),n&&this.currentPaymentMethod===3&&(n.classList.add("hidden"),n.classList.remove("md:hidden"))}else{e&&e.setAttribute("src",m);let o=document.querySelector(".payment-options");o&&(o.innerHTML=`<div> 
        ${Q.map((l,d)=>`
              <div class="tab">   
                <button class="tab-button group" data-tab="${d}"> 
            ...
            `).join("")}
        </div>`);const C=document.querySelectorAll(".tab-button");Q.forEach((l,d)=>{const p=C[d];if(p){const H=p.querySelector(".tab-button-icon"),S=p.querySelector(".tab-button-label");H&&(H.innerHTML=l.icon),S&&(S.textContent=l.label),p.dataset.tab=d.toString(),p.addEventListener("click",()=>{this.setCurrentPaymentMethod(d)})}})}this.displayTabLayout(),i&&this.displayPaymentWarningText(this.currentPaymentMethod,i)}mobileContainerContent(){return`  <div class="box-container" id="inner-mob-container">
      ...
      ${Q.map((t,n)=>`
              ...
              `).join("")}`}displayPaymentWarningText(e,t){const n=()=>{switch(e){case 0:return"Incorrect otp. please retry with the correct otp";case 1:return"Account expired";case 2:return"Please dial the ussd shortcode";default:return"Error"}};t&&(t.innerHTML=n())}}return s1});
