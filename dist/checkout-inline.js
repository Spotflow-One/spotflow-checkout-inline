...
`,key:"ussd"}],$=[{name:"First City Monument Bank",code:"*329#"},{name:"First Bank",code:"*268#"}],B=[{name:"MasterCard",icon:`<svg width="24" height="14" viewBox="0 0 24 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`},{name:"American Express",icon:`<svg width="25" height="14" viewBox="0 0 25 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`},{name:"UnionPay",icon:`<svg width="30" height="22" viewBox="0 0 30 22" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`},{name:"Discover",icon:`<svg width="25" height="14" viewBox="0 0 25 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`},{name:"Verve",icon:`<svg width="28" height="14" viewBox="0 0 28 14" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`},{name:"Visa",icon:`<svg width="28" height="7" viewBox="0 0 28 7" fill="none" xmlns="http://www.w3.org/2000/svg">
...
`}];class F{constructor(e,t,n,A,s){o(this,"cardDetailsValues");o(this,"cardPin");o(this,"cardOtp");o(this,"contents");o(this,"container");o(this,"closeModal");o(this,"email");o(this,"token");o(this,"amount");o(this,"_currentStep");o(this,"activeRef");o(this,"creditCardTypes");this.container=e,this.closeModal=t,this.token=n,this.email=A,this.amount=s,this._currentStep=1,this.cardDetailsValues={number:"",expiry:"",cvv:""},this.cardPin="",this.cardOtp="",this.activeRef="",this.contents=document.querySelectorAll(".content"),this.creditCardTypes=[...B],this.renderCardContent(),this.currentStep===1&&this.displayCardTypes(),this.attachInputListeners()}displayCardTypes(){requestAnimationFrame(()=>{const e=this.container.querySelector("#card-types");this.creditCardTypes.length===1&&this.creditCardTypes[0].name==="Unknown"?B.forEach(t=>{const n=document.createElement("div");n.classList.add("card-type"),n.innerHTML=t.icon,e.appendChild(n)}):(e.innerHTML="",this.creditCardTypes.forEach(t=>{const n=document.createElement("div");n.classList.add("card-type"),n.innerHTML=t.icon,e.appendChild(n)}))})}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderCardContent(),e===1&&this.displayCardTypes()}attachInputListeners(){const e=this.container.querySelector('input[name="number"]'),t=this.container.querySelector('input[name="expiry"]'),n=this.container.querySelector('input[name="cvv"]'),A=this.container.querySelector(".details-form-button"),s=document.querySelector("#checkoutcard"),i=Array.from(this.container.querySelectorAll(".pin-input")),c=this.container.querySelector(".otp-input"),a=this.container.querySelector(".otp-button"),d=this.container.querySelector(".success-button");e&&e.addEventListener("input",l=>this.handleInputChange(l,A)),t&&t.addEventListener("input",l=>this.handleInputChange(l,A)),n&&n.addEventListener("input",l=>this.handleInputChange(l,A)),s&&s.addEventListener("submit",l=>this.handleSubmit(l)),i.forEach((l,h)=>{l.addEventListener("input",p=>this.handlePinInputChange(p,h,i)),l.addEventListener("paste",p=>this.handlePinPaste(p,i))}),c&&c.addEventListener("input",l=>this.handleOtpInput(l,a)),a&&a.addEventListener("click",l=>this.submitOtp(l)),d&&d.addEventListener("click",()=>this.closeModal())}handleInputChange(e,t){const n=e.target,{name:A,value:s}=n;let i=s;A==="number"?(i=z(s),this.filterCreditCardType(s)):A==="expiry"?i=P(s):A==="cvv"&&(i=V(s)),this.cardDetailsValues={...this.cardDetailsValues,[A]:i},n.value=i,this.cardDetailsValues.number&&this.cardDetailsValues.expiry&&this.cardDetailsValues.cvv&&t&&t.removeAttribute("disabled")}handlePinRequest(){const e=this.container.querySelector("#loader"),t=this.container.querySelector("#pin-content");if(this.cardPin.length===4){const n={authorization:{pin:this.cardPin},reference:this.activeRef};setTimeout(()=>{var s;e&&t&&(e.classList.remove("hidden"),(s=e.querySelector("img"))==null||s.setAttribute("src",f),t.classList.add("hidden")),N(this.token,n).then(i=>{i.status==="failed"?i.providerMessage?C(i.providerMessage,"error"):C("Payment failed","error"):(C("Payment Authorized!","success"),this.currentStep=3)}).catch(i=>{C(i.message,"error")}).finally(()=>{e&&t&&(e.classList.add("hidden"),t.classList.remove("hidden"))})},500)}}handlePinInputChange(e,t,n){this.cardPin="",e.target.value?t<n.length-1&&n[t+1].focus():t>0&&n[t-1].focus(),this.cardPin=n.map(i=>i.value).join(""),this.handlePinRequest()}showLoader(){var t;const e=this.container.querySelector("#loader");e&&(e.classList.remove("hidden"),(t=e.querySelector("img"))==null||t.setAttribute("src",f))}handlePinPaste(e,t){var s;e.preventDefault(),this.cardPin="";const A=(((s=e.clipboardData)==null?void 0:s.getData("text").trim())||"").split("");t.forEach((i,c)=>{i.value=A[c]||""});for(let i=0;i<t.length;i++)if(!t[i].value){t[i].focus();break}this.cardPin=t.map(i=>i.value).join(""),this.handlePinRequest()}handleOtpInput(e,t){const n=e.target,{value:A}=n;this.cardOtp="",this.cardOtp=A,this.cardOtp.length>3&&t&&t.removeAttribute("disabled")}submitOtp(e){var i;e.preventDefault();const t=this.container.querySelector("#loader"),n=this.container.querySelector("#otp-content"),A={authorization:{otp:this.cardPin},reference:this.activeRef};t&&n&&(t.classList.remove("hidden"),(i=t.querySelector("img"))==null||i.setAttribute("src",f),n.classList.add("hidden")),q(this.token,A).then(c=>{c.status==="failed"?c.providerMessage?C(c.providerMessage,"error"):C("Payment failed","error"):c.status==="successful"&&(this.currentStep=5)}).catch(c=>{C(c.message,"error")}).finally(()=>{t&&n&&(t.classList.add("hidden"),n.classList.remove("hidden"))})}async handleSubmit(e){var i,c;e.preventDefault();const t=this.container.querySelector(".spinner"),n=this.container.querySelector("#details-form-button-text"),A={amount:this.amount,channel:"card",currency:"USD",customer:{email:this.email},reference:k(),card:{pan:W(this.cardDetailsValues.number),cvv:this.cardDetailsValues.cvv,expiryMonth:(i=this.cardDetailsValues)==null?void 0:i.expiry.split("/")[0].trim(),expiryYear:(c=this.cardDetailsValues)==null?void 0:c.expiry.split("/")[1].trim()}};t&&n&&(t.classList.remove("hidden"),n.classList.add("hidden")),Z(this.token,A).then(a=>{a.status==="failed"?a.providerMessage?C(a.providerMessage,"error"):C("Payment failed","error"):a.status==="pending_authorization"?(C("Payment Created!","success"),this.activeRef=a.reference,this.currentStep=2):a.status==="pending_validation"&&a.authorization.mode==="3DS"&&a.authorization.redirectUrl&&(this.activeRef=a.reference,this.redirectTo3DS(a.authorization.redirectUrl))}).catch(a=>{C(a.message,"error")}).finally(()=>{t&&n&&(t.classList.add("hidden"),n.classList.remove("hidden"))})}redirectTo3DS(e){const t=window.open(e,"_blank");if(t){this.currentStep=6,this.showLoader();const n=setInterval(()=>{t.closed&&(clearInterval(n),this.verifyPayment(()=>{this.currentStep=5},A=>{C(A.message,"error"),this.currentStep=4}))},2e3)}else alert("Please enable popups for this site to complete the authentication.")}async verifyPayment(e,t){try{const n=await I(this.token,this.activeRef);n.status==="failed"?this.currentStep=4:e(n)}catch(n){console.error(n),t(new Error("failed"))}}getCardStepContent(){switch(this.currentStep){case 1:return R;case 2:return J;case 3:return K;case 4:return b;case 5:return x;case 6:return O;default:return R}}filterCreditCardType(e){e?(this.creditCardTypes=B.filter(t=>t.name===j(e)),this.creditCardTypes.length>=1?this.displayCardTypes():(this.creditCardTypes=[...B],this.displayCardTypes())):(this.creditCardTypes=[...B],this.displayCardTypes())}renderCardContent(){this.contents[0]&&(this.contents[0].innerHTML=this.getCardStepContent(),this.attachInputListeners())}}const G=`<div>
  <div class="box-container">
    <div class="container-center h-[40vh] hidden" id="loader">
      <img src="" alt="loader gif" />
    </div>
...
`,_=`<div id="paymentWarning">
...
</div>`,t1=`<div id="trasferConfirmationOne">
    <div class="box-container">
    ...
    </div>`,e1=`<div id="trasferConfirmationOne">
  <div class="box-container">
    <div class="text-center mt-7 w-4/5 md:w-3/5 mx-auto">
      <h5 class="text-center text-grey-400 w-4/5 mx-auto mt-4 mb-7 flex">
...
`,n1=`<div id="trasferConfirmationProgress">
  <div class="box-container">
...
`;class A1{constructor(e,t,n,A,s){o(this,"container");o(this,"contents");o(this,"closeModal");o(this,"email");o(this,"token");o(this,"amount");o(this,"_currentStep");o(this,"activeRef");o(this,"paymentResponse");o(this,"initialTime");o(this,"timeLeft");o(this,"timerId");o(this,"abortController",null);this.container=e,this.contents=document.querySelectorAll(".content"),this.closeModal=t,this.token=n,this.email=A,this.amount=s,this._currentStep=1,this.renderTransferContent(),this.paymentResponse=null,this.activeRef="",this.initialTime=600,this.timeLeft=this.initialTime,this.timerId=null,this.attachInputListeners(),this.currentStep===1&&(this.abortController=new AbortController,this.createTransfer())}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderTransferContent(),e===1&&this.createTransfer(),e===5&&this.startTimer("check")}attachInputListeners(){const e=this.container.querySelector("#transfer-acn"),t=this.container.querySelector("#copy-acn"),n=this.container.querySelector("#acn-tooltip"),A=this.container.querySelector("#transfer-amount"),s=this.container.querySelector("#copy-amount"),i=this.container.querySelector("#amount-tooltip"),c=this.container.querySelector("#transfer-try-again"),a=this.container.querySelector("#transfer-sent"),d=this.container.querySelector(".transfer-button"),l=this.container.querySelector("#show-acn"),h=this.container.querySelector("#ctd-timer"),p=this.container.querySelector(".success-button"),S=this.container.querySelector("#confirm-one"),H=this.container.querySelector(".transfer-close");if(t&&e&&t.addEventListener("click",()=>{n&&this.copyToClipboard(e.innerHTML,n)}),s&&A&&s.addEventListener("click",()=>{i&&this.copyToClipboard(A.innerHTML.split(" ")[1],i)}),d&&d.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),c&&c.addEventListener("click",()=>{this.currentStep=1}),a&&a.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),l&&l.addEventListener("click",()=>{this.stopProgressBar(),this.currentStep=1}),this.currentStep===5){this.initialTime=300,this.timeLeft=this.initialTime,h&&(h.innerHTML=M(this.initialTime));const a1=3e4;this.abortController=new AbortController,setTimeout(()=>{this.verifyTransfer(this.token,a1,()=>{this.currentStep=6},D=>{console.error("Polling failed:",D),C(D.message,"error")})},5e3),this.startTimer("check")}S&&S.addEventListener("click",()=>{this.currentStep=4}),p&&p.addEventListener("click",()=>this.closeModal()),H&&H.addEventListener("click",()=>this.closeModal())}async createTransfer(){requestAnimationFrame(()=>{var s;const e=this.container.querySelector("#loader"),t=this.container.querySelector("#details-content");e&&t&&(e.classList.remove("hidden"),(s=e.querySelector("img"))==null||s.setAttribute("src",f),t.classList.add("hidden"));const n={amount:this.amount,channel:"bank_transfer",currency:"USD",customer:{email:this.email},reference:k()};Y(this.token,n).then(i=>{if(i.status==="failed")i.providerMessage?C(i.providerMessage,"error"):C("Payment failed","error"),this.currentStep=2;else{e&&t&&(e.classList.add("hidden"),t.classList.remove("hidden")),this.activeRef=i.reference,this.paymentResponse=i,this.updateTransferDetailsContent();const c=3e4;setTimeout(()=>{this.verifyTransfer(this.token,c,()=>{this.currentStep=6},a=>{console.error("Polling failed:",a),C(a.message,"error")})},c)}}).catch(i=>{C(i.message,"error")})})}async verifyTransfer(e,t,n,A){let s=0,i=t,c=!0,a;this.abortController&&(a=this.abortController.signal);const d=async()=>{if(this.timeLeft<=0){c=!1,A(new Error("Max polling time reached")),this.currentStep=2;return}s+=1;try{const l=await I(e,this.activeRef,a);l.status==="failed"?c&&(i=t*(s+1),setTimeout(d,i)):(c=!1,n(l))}catch{c&&(i=t*(s+1),setTimeout(d,i))}};d()}getTransferStepContent(){switch(this.currentStep){case 1:return G;case 2:return _;case 3:return t1;case 4:return e1;case 5:return n1;case 6:return x;default:return G}}updateTransferDetailsContent(){var s;const e=this.container.querySelectorAll(".transfer-curr-amount"),t=this.container.querySelector("#transfer-bank"),n=this.container.querySelector("#transfer-acn"),A=this.container.querySelector("#time-left");this.paymentResponse&&(e.length&&e.forEach(i=>{var c,a;return i.innerHTML=`${(c=this.paymentResponse)==null?void 0:c.currency} ${(a=this.paymentResponse)==null?void 0:a.amount}`}),t&&this.paymentResponse.bankDetails&&(t.innerHTML=(s=this.paymentResponse.bankDetails)==null?void 0:s.bankName),n&&this.paymentResponse.bankDetails&&(n.innerHTML=this.paymentResponse.bankDetails.accountNumber),A&&(A.innerHTML=M(this.initialTime)),this.startTimer("poll"))}renderTransferContent(){this.contents[1]&&(this.contents[1].innerHTML=this.getTransferStepContent(),this.attachInputListeners())}startTimer(e){const t=this.container.querySelector("#ctd-timer");this.stopTimer(),this.timerId=window.setInterval(()=>{this.timeLeft>0?(this.timeLeft-=1,e==="poll"?this.updateProgressTimerContent():t&&(t.innerHTML=M(this.timeLeft),this.startTimer("check"))):(this.stopTimer(),e==="check"&&(this.currentStep=3))},1e3)}updateProgressTimerContent(){const t=2*Math.PI*40,A=(this.initialTime-this.timeLeft)/this.initialTime*100/100*t,s=this.container.querySelector(".circle-progress"),i=this.container.querySelector("#time-left");s&&(s.setAttribute("stroke-dasharray",`${t}`),s.setAttribute("stroke-dashoffset",`${A}`)),i&&(i.innerHTML=M(this.timeLeft))}stopTimer(){this.timerId!==null&&(clearInterval(this.timerId),this.timerId=null)}stopProgressBar(){const e=this.container.querySelector(".progress-bar");e&&(e.style.animationPlayState="paused")}copyToClipboard(e,t){navigator.clipboard.writeText(e).then(()=>{t.innerHTML="Copied!"}).catch(()=>{t.innerHTML="Failed to copy"})}destroyTimer(){this.stopTimer()}stopPolling(){this.abortController&&this.abortController.abort()}}const U=`<div id="ussdBanks">
  <div class="box-container">
...
`;class i1{constructor(){o(this,"contents");o(this,"banks");o(this,"_currentStep");this.contents=document.querySelectorAll(".content"),this.banks=$,this._currentStep=1,this.renderUssdContent()}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderUssdContent()}getUssdStepContent(){switch(this.currentStep){case 1:return U;case 2:return s1;case 3:return b;case 4:return x;default:return U}}renderUssdContent(){this.contents[2]&&(this.contents[2].innerHTML=this.getUssdStepContent())}renderSearchOptions(e,t,n){n.innerHTML="",e.forEach(A=>{const s=document.createElement("div");s.classList.add("bank-option"),s.textContent=A.name,s.addEventListener("click",()=>{t.value=A.name,n.style.display="none"}),n.appendChild(s)}),n.style.display=e.length?"block":"none"}searchBanksOnInput(e,t){const n=e.value.toLowerCase(),A=this.banks.filter(s=>s.name.toLowerCase().includes(n));this.renderSearchOptions(A,e,t)}openSearchOnFocus(e,t){this.renderSearchOptions(this.banks,e,t)}closeSearchOptionsOutsideFocus(e,t,n){!t.contains(e.target)&&!n.contains(e.target)&&(n.style.display="none")}}class r1{constructor(e,t,n){o(this,"modalContainer");o(this,"currentPaymentMethod");o(this,"currentStep");o(this,"container");o(this,"card");o(this,"transfer");o(this,"ussd");o(this,"merchantKey");o(this,"email");o(this,"amount");o(this,"isMobile",!1);if(this.modalContainer=document.createElement("div"),this.modalContainer.innerHTML=m,document.body.appendChild(this.modalContainer),this.isMobile=window.matchMedia("(max-width: 767px)").matches,this.currentPaymentMethod=this.isMobile?3:0,this.currentStep=1,this.container=document.querySelectorAll(".content")[0],this.merchantKey=e,this.email=t,this.amount=n,this.attachInputListeners(),e===""||t==="")throw this.closeModal(),new Error("Merchant key or email is required")}attachInputListeners(){const e=document.body.querySelector("#change-method"),t=document.body.querySelectorAll(".close-btn"),n=document.querySelector("#mob-container"),A=document.querySelector("#mob-tab-co"),s=document.querySelector("#mob-tab-c");e&&(this.cleanup(),e.addEventListener("click",()=>{this.currentPaymentMethod=3,this.attachInputListeners(),n&&(n.innerHTML=this.mobileContainerContent()),this.setup(),A&&s&&s.parentNode&&A.removeChild(s)})),t.length>0&&t.forEach(i=>{i.addEventListener("click",()=>{this.cleanup(),this.closeModal()})})}displayTabLayout(){const e=document.querySelectorAll(".tab-button"),t=document.querySelectorAll(".mob-tab-button"),n=document.querySelector(".box-container-method"),A=document.querySelector("#mob-container"),s=document.querySelector("#inner-mob-container"),i=document.querySelector("#mob-tab-co");this.isMobile?t&&n&&A&&s&&t.forEach(c=>{c.addEventListener("click",a=>{if(a.currentTarget instanceof HTMLElement&&a.currentTarget.dataset.tab){const d=parseInt(a.currentTarget.dataset.tab);this.updatePaymentMethodView(this.merchantKey,this.email,this.amount),this.setCurrentPaymentMethod(d),A.removeChild(s),this.cleanup(),Q.forEach((l,h)=>{this.currentPaymentMethod===h&&i&&(i.innerHTML=`
                          <div id="mob-tab-c" class="w-full pt-2 px-5 md:hidden">
                            <div>
                              <button class="pt-3 px-2 grow align-center text-grey-600 tex-sm font-semibold" data-tab="${h}">
                      ...
                      `)}),this.renderPaymentMethodContent(),this.attachInputListeners()}})}):e&&n&&e.forEach(c=>{c.addEventListener("click",a=>{if(a.currentTarget instanceof HTMLElement&&a.currentTarget.dataset.tab){const d=parseInt(a.currentTarget.dataset.tab);this.updatePaymentMethodView(this.merchantKey,this.email,this.amount),this.setCurrentPaymentMethod(d),this.cleanup(),this.renderPaymentMethodContent(),this.attachInputListeners()}})}),this.updatePaymentMethodView(this.merchantKey,this.email,this.amount),this.renderPaymentMethodContent(),this.attachInputListeners(),this.setCurrentPaymentMethod(this.currentPaymentMethod)}updatePaymentMethodView(e,t,n){switch(this.currentPaymentMethod){case 0:return this.card=new F(this.modalContainer,()=>this.closeModal(),e,t,n);case 1:return this.transfer=new A1(this.modalContainer,()=>this.closeModal(),e,t,n);case 2:return this.ussd=new i1;default:return this.card=new F(this.modalContainer,()=>this.closeModal(),e,t,n)}}renderPaymentMethodContent(){return(()=>{switch(this.currentPaymentMethod){case 0:return this.card.renderCardContent();case 1:return this.transfer.renderTransferContent();case 2:return this.ussd.renderUssdContent();case 3:return"";default:return this.card.renderCardContent()}})()}setCurrentPaymentMethod(e){this.currentPaymentMethod=e;const t=document.querySelectorAll(".tab-button");let n=document.querySelectorAll(".content");const A=document.getElementById("mob-action-btns");t.forEach(s=>s.classList.remove("active")),n.forEach(s=>s.classList.remove("active")),t[e]&&t[e].classList.add("active"),n[e]&&n[e].classList.add("active"),e!==3&&A&&(A.classList.remove("hidden"),A.classList.add("md:hidden"))}cleanup(){this.transfer&&this.transfer.destroyTimer()}closeModal(){this.modalContainer&&this.modalContainer.parentNode&&document.body.removeChild(this.modalContainer)}setup(){const e=document.getElementById("merchantLogo"),t=document.getElementById("merchantLogoMob"),n=document.getElementById("mob-action-btns"),A=document.getElementById("merchant-email"),s=this.modalContainer.querySelector("#paymentWarning"),i=s==null?void 0:s.querySelector("#payment-warning-text"),c=document.querySelector("#mob-container");if(A&&(A.innerHTML=this.email),this.isMobile){t&&t.setAttribute("src",g),c&&(c.innerHTML=this.mobileContainerContent());let a=document.querySelector(".mob-options");a&&this.viewMobileOptions(a);const d=document.querySelectorAll(".mob-tab-button");Q.forEach((l,h)=>{const p=d[h];p&&(p.dataset.tab=h.toString(),p.addEventListener("click",()=>{this.setCurrentPaymentMethod(h)}))}),n&&this.currentPaymentMethod===3&&(n.classList.add("hidden"),n.classList.remove("md:hidden"))}else{e&&e.setAttribute("src",g);let a=document.querySelector(".payment-options");a&&(a.innerHTML=`<div> 
        ${Q.map((l,h)=>`
              <div class="tab">   
                <button class="tab-button group" data-tab="${h}"> 
            ...
            `).join("")}
        </div>`);const d=document.querySelectorAll(".tab-button");Q.forEach((l,h)=>{const p=d[h];if(p){const S=p.querySelector(".tab-button-icon"),H=p.querySelector(".tab-button-label");S&&(S.innerHTML=l.icon),H&&(H.textContent=l.label),p.dataset.tab=h.toString(),p.addEventListener("click",()=>{this.setCurrentPaymentMethod(h)})}})}this.displayTabLayout(),i&&this.displayPaymentWarningText(this.currentPaymentMethod,i)}mobileContainerContent(){return`  <div class="box-container" id="inner-mob-container">
              ...
              </div>`}viewMobileOptions(e){e.innerHTML=`
      ${Q.map((t,n)=>`
                <div class="tab">
              ...
              `).join("")}`}displayPaymentWarningText(e,t){const n=()=>{switch(e){case 0:return"Incorrect otp. please retry with the correct otp";case 1:return"Account expired";case 2:return"Please dial the ussd shortcode";default:return"Error"}};t&&(t.innerHTML=n())}}w.CheckoutForm=r1,Object.defineProperty(w,Symbol.toStringTag,{value:"Module"})});
