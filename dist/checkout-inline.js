...
var G = (r, A, t) => A in r ? T(r, A, { enumerable: !0, configurable: !0, writable: !0, value: t }) : r[A] = t;
var a = (r, A, t) => G(r, typeof A != "symbol" ? A + "" : A, t);
...
class h extends Error {
  constructor(A, t) {
    super(A), this.status = t, this.name = "FetchError";
  }
}
class w extends Error {
  constructor(A) {
    super(A), this.name = "AuthorizationError";
  }
}
const u = (r) => {
  if (!r)
    throw new w("Missing authorization token");
  const A = new Headers();
  return A.append("Content-Type", "application/json"), A.append("Authorization", `Bearer ${r}`), A;
}, E = "http://dev-api.spotflow.one/api/v1", O = async (r, A) => {
  try {
    const t = u(r), e = await fetch(`${E}/payments`, {
      method: "POST",
      headers: t,
    ...
    if (!e.ok) {
      ...
      throw i.message ? new h(i.message, e.status) : new h("Network response was not ok", e.status);
    }
    return await e.json();
  } catch (t) {
    throw t instanceof h ? console.error(`FetchError: ${t.message} (status: ${t.status})`) : t instanceof w ? console.error(`AuthorizationError: ${t.message}`) : console.error(`Unexpected error: ${t}`), t;
  }
}, N = async (r, A) => {
  try {
    const t = u(r), e = await fetch(`${E}/payments/authorize`, {
      method: "POST",
      headers: t,
      body: JSON.stringify(A)
    });
  ...
  try {
    const t = u(r), e = await fetch(`${E}/payments/validate`, {
      method: "POST",
      headers: t,
      body: JSON.stringify(A)
    });
  ...
  try {
    const e = u(r), n = await fetch(`${E}/payments/verify?reference=${A}`, {
      method: "GET",
      headers: e,
      signal: t
    });
    ...
    throw e instanceof h ? console.error(`FetchError: ${e.message} (status: ${e.status})`) : e instanceof w ? console.error(`AuthorizationError: ${e.message}`) : console.error(`Unexpected error: ${e}`), e;
  }
}, H = () => {
  const r = Date.now().toString(36), A = "xxxx-xxxx-xxx-xxxx".replace(/[x]/g, () => Math.floor(Math.random() * 16).toString(16));
  return `SPF-${r}-${A}`.toUpperCase();
}, m = (r = "") => r.replace(/\D+/g, ""), P = (r) => {
  if (!r)
    return r;
  const A = m(r);
  return `${A.slice(0, 4)} ${A.slice(
    4,
    8
  )} ${A.slice(8, 12)} ${A.slice(12, 16)}`.trim();
}, Z = (r) => m(r).slice(0, 3), W = (r) => {
  const A = m(r);
  return A.length >= 3 ? `${A.slice(0, 2)} / ${A.slice(2, 4)}` : A;
}, V = (r) => {
  if (!r)
    return r;
  const A = m(r);
  ...
  t.classList.add("toast", A);
  const e = document.createElement("span");
  e.textContent = r, t.appendChild(e);
  ...
  if (e) {
    const n = j(r, A);
    e.appendChild(n), n.offsetHeight, n.classList.add("show"), setTimeout(() => F(n), t);
  }
}, F = (r) => {
...
class M {
  constructor(A, t, e, n) {
    ...
    a(this, "_currentStep");
    a(this, "activeRef");
    this.container = A, this.closeModal = t, this.token = e, this.email = n, this._currentStep = 1, this.cardDetailsValues = {
      number: "",
      expiry: "",
      cvv: ""
  ...
  set currentStep(A) {
    this._currentStep = A, this.renderCardContent();
  }
  attachInputListeners() {
    const A = this.container.querySelector(
      ...
      ".details-form-button"
    ), i = document.querySelector("#checkoutcard"), s = Array.from(
    ...
    A && A.addEventListener(
      ...
      "input",
      (l) => this.handleInputChange(l, n)
    ), i && i.addEventListener("submit", (l) => this.handleSubmit(l)), s.forEach((l, g) => {
      l.addEventListener(
        "input",
        (p) => this.handlePinInputChange(p, g, s)
      ), l.addEventListener(
        "paste",
        (p) => this.handlePinPaste(p, s)
      );
    }), o && o.addEventListener(
      "input",
      (l) => this.handleOtpInput(l, c)
    ), c && c.addEventListener("click", (l) => this.submitOtp(l)), C && C.addEventListener("click", () => this.closeModal());
  }
  handleInputChange(A, t) {
    const e = A.target, { name: n, value: i } = e;
    let s = i;
    n === "number" ? s = P(i) : n === "expiry" ? s = W(i) : n === "cvv" && (s = Z(i)), this.cardDetailsValues = {
      ...this.cardDetailsValues,
      [n]: s
    }, e.value = s, this.cardDetailsValues.number && this.cardDetailsValues.expiry && this.cardDetailsValues.cvv && t && t.removeAttribute("disabled");
  }
  handlePinRequest() {
    const A = this.container.querySelector("#loader"), t = this.container.querySelector("#pin-content");
    if (this.cardPin.length === 4) {
      ...
      setTimeout(() => {
        var i;
        A && t && (A.classList.remove("hidden"), (i = A.querySelector("img")) == null || i.setAttribute("src", y), t.classList.add("hidden")), N(this.token, e).then((s) => {
          s.status === "failed" ? s.providerMessage ? d(s.providerMessage, "error") : d("Payment failed", "error") : (d("Payment Authorized!", "success"), this.currentStep = 3);
        }).catch((s) => {
          d(s.message, "error");
        }).finally(() => {
          A && t && (A.classList.add("hidden"), t.classList.remove("hidden"));
        });
      }, 500);
    }
  }
  handlePinInputChange(A, t, e) {
    this.cardPin = "", A.target.value ? t < e.length - 1 && e[t + 1].focus() : t > 0 && e[t - 1].focus(), this.cardPin = e.map((s) => s.value).join(""), this.handlePinRequest();
  }
  handlePinPaste(A, t) {
    var i;
    A.preventDefault(), this.cardPin = "";
    const n = (((i = A.clipboardData) == null ? void 0 : i.getData("text").trim()) || "").split("");
    t.forEach((s, o) => {
      s.value = n[o] || "";
    });
    ...
    this.cardPin = t.map((s) => s.value).join(""), this.handlePinRequest();
  }
  handleOtpInput(A, t) {
    const e = A.target, { value: n } = e;
    this.cardOtp = "", this.cardOtp = n, this.cardOtp.length > 3 && t && t.removeAttribute("disabled");
  }
  submitOtp(A) {
    var s;
    A.preventDefault();
    const t = this.container.querySelector("#loader"), e = this.container.querySelector("#otp-content"), n = {
      authorization: {
        otp: this.cardPin
      },
      reference: this.activeRef
    };
    t && e && (t.classList.remove("hidden"), (s = t.querySelector("img")) == null || s.setAttribute("src", y), e.classList.add("hidden")), Y(this.token, n).then((o) => {
      ...
      t && e && (t.classList.add("hidden"), e.classList.remove("hidden"));
    });
  }
  async handleSubmit(A) {
    var s, o;
    A.preventDefault();
    const t = this.container.querySelector(".spinner"), e = this.container.querySelector(
      ...
      reference: H(),
      card: {
        pan: V(this.cardDetailsValues.number),
        cvv: this.cardDetailsValues.cvv,
        expiryMonth: (s = this.cardDetailsValues) == null ? void 0 : s.expiry.split("/")[0].trim(),
        expiryYear: (o = this.cardDetailsValues) == null ? void 0 : o.expiry.split("/")[1].trim()
      }
    };
    t && e && (t.classList.remove("hidden"), e.classList.add("hidden")), O(this.token, n).then((c) => {
...
class At {
  constructor(A, t, e, n) {
    ...
    this.container = A, this.contents = document.querySelectorAll(".content"), this.closeModal = t, this.token = e, this.email = n, this._currentStep = 1, this.renderTransferContent(), this.paymentResponse = null, this.activeRef = "", this.initialTime = 600, this.timeLeft = this.initialTime, this.timerId = null, this.attachInputListeners(), this.currentStep === 1 && (this.abortController = new AbortController(), this.createTransfer());
  }
  get currentStep() {
    return this._currentStep;
  }
  set currentStep(A) {
    this._currentStep = A, this.renderTransferContent(), A === 1 && this.createTransfer(), A === 5 && this.startTimer("check");
  }
  attachInputListeners() {
    const A = this.container.querySelector("#transfer-acn"), t = this.container.querySelector("#copy-acn"), e = this.container.querySelector("#acn-tooltip"), n = this.container.querySelector("#transfer-amount"), i = this.container.querySelector("#copy-amount"), s = this.container.querySelector("#amount-tooltip"), o = this.container.querySelector("#transfer-try-again"), c = this.container.querySelector("#transfer-sent"), C = this.container.querySelector(".transfer-button"), l = this.container.querySelector("#show-acn"), g = this.container.querySelector("#ctd-timer"), p = this.container.querySelector(
      ".success-button"
    ), S = this.container.querySelector("#confirm-one"), x = this.container.querySelector(".transfer-close");
    if (t && A && t.addEventListener("click", () => {
      e && this.copyToClipboard(A.innerHTML, e);
    }), i && n && i.addEventListener("click", () => {
      s && this.copyToClipboard(n.innerHTML.split(" ")[1], s);
      ...
      this.initialTime = 300, this.timeLeft = this.initialTime, g && (g.innerHTML = Q(this.initialTime));
      const b = 3e4;
    ...
    S && S.addEventListener("click", () => {
      this.currentStep = 4;
    }), p && p.addEventListener("click", () => this.closeModal()), x && x.addEventListener("click", () => this.closeModal());
  }
  async createTransfer() {
    requestAnimationFrame(() => {
      var i;
      const A = this.container.querySelector("#loader"), t = this.container.querySelector("#details-content");
      A && t && (A.classList.remove("hidden"), (i = A.querySelector("img")) == null || i.setAttribute("src", y), t.classList.add("hidden"));
      const e = {
        amount: 14.99,
      ...
      z(this.token, e).then((s) => {
        if (s.status === "failed")
          s.providerMessage ? d(s.providerMessage, "error") : d("Payment failed", "error"), this.currentStep = 2;
        else {
          A && t && (A.classList.add("hidden"), t.classList.remove("hidden")), this.activeRef = s.reference, this.paymentResponse = s, this.updateTransferDetailsContent();
          const o = 3e4;
          setTimeout(() => {
            this.verifyTransfer(
              this.token,
  ...
  async verifyTransfer(A, t, e, n) {
    ...
    const C = async () => {
      ...
      try {
        ...
        l.status === "failed" ? o && (s = t * (i + 1), setTimeout(C, s)) : (o = !1, e(l));
      } catch {
        o && (s = t * (i + 1), setTimeout(C, s));
      }
    };
    C();
  }
  ...
  updateTransferDetailsContent() {
    var i;
    const A = this.container.querySelectorAll(".transfer-curr-amount"), t = this.container.querySelector("#transfer-bank"), e = this.container.querySelector("#transfer-acn"), n = this.container.querySelector("#time-left");
    this.paymentResponse && (A.length && A.forEach(
      (s) => {
        var o, c;
        return s.innerHTML = `${(o = this.paymentResponse) == null ? void 0 : o.currency} ${(c = this.paymentResponse) == null ? void 0 : c.amount}`;
      }
    ), t && this.paymentResponse.bankDetails && (t.innerHTML = (i = this.paymentResponse.bankDetails) == null ? void 0 : i.bankName), e && this.paymentResponse.bankDetails && (e.innerHTML = this.paymentResponse.bankDetails.accountNumber), n && (n.innerHTML = Q(this.initialTime)), this.startTimer("poll"));
  }
  ...
  startTimer(A) {
    ...
    this.stopTimer(), this.timerId = window.setInterval(() => {
      this.timeLeft > 0 ? (this.timeLeft -= 1, A === "poll" ? this.updateProgressTimerContent() : t && (t.innerHTML = Q(this.timeLeft), this.startTimer("check"))) : (this.stopTimer(), A === "check" && (this.currentStep = 3));
    }, 1e3);
  }
  updateProgressTimerContent() {
    const t = 2 * Math.PI * 40, n = (this.initialTime - this.timeLeft) / this.initialTime * 100 / 100 * t, i = this.container.querySelector(".circle-progress"), s = this.container.querySelector("#time-left");
    i && (i.setAttribute("stroke-dasharray", `${t}`), i.setAttribute("stroke-dashoffset", `${n}`)), s && (s.innerHTML = Q(this.timeLeft));
  }
  stopTimer() {
  ...
  stopProgressBar() {
    ...
    A && (A.style.animationPlayState = "paused");
  }
  copyToClipboard(A, t) {
...
class st {
  ...
  set currentStep(A) {
    this._currentStep = A, this.renderUssdContent();
  }
  getUssdStepContent() {
    switch (this.currentStep) {
      case 1:
  ...
  renderSearchOptions(A, t, e) {
    e.innerHTML = "", A.forEach((n) => {
      const i = document.createElement("div");
      i.classList.add("bank-option"), i.textContent = n.name, i.addEventListener("click", () => {
        t.value = n.name, e.style.display = "none";
      }), e.appendChild(i);
    }), e.style.display = A.length ? "block" : "none";
  }
  searchBanksOnInput(A, t) {
    const e = A.value.toLowerCase(), n = this.banks.filter(
      (i) => i.name.toLowerCase().includes(e)
    );
    this.renderSearchOptions(n, A, t);
  }
  openSearchOnFocus(A, t) {
    this.renderSearchOptions(this.banks, A, t);
  }
  closeSearchOptionsOutsideFocus(A, t, e) {
    !t.contains(A.target) && !e.contains(A.target) && (e.style.display = "none");
  }
}
class rt {
  constructor(A, t) {
    ...
    a(this, "merchantKey");
    a(this, "email");
    this.modalContainer = document.createElement("div"), this.modalContainer.innerHTML = U, document.body.appendChild(this.modalContainer), this.currentPaymentMethod = 0, this.currentStep = 1, this.container = document.querySelectorAll(".content")[0], this.merchantKey = A, this.email = t, this.updatePaymentMethodView(this.merchantKey, this.email), this.attachInputListeners();
  }
  attachInputListeners() {
    const A = this.modalContainer.querySelector(
      ".success-button"
    ), t = this.modalContainer.querySelector(
      "#close-modal"
    );
    A && (this.cleanup(), A.addEventListener("click", () => {
      document.body.removeChild(this.modalContainer);
    })), t && (this.cleanup(), t.addEventListener("click", () => {
      document.body.removeChild(this.modalContainer);
    }));
  }
  displayTabLayout() {
    const A = document.querySelectorAll(".tab-button"), t = document.querySelector(".box-container-method");
    A && t && A.forEach((e) => {
      e.addEventListener("click", (n) => {
        if (n.currentTarget instanceof HTMLElement && n.currentTarget.dataset.tab) {
          const i = parseInt(n.currentTarget.dataset.tab);
          this.updatePaymentMethodView(this.merchantKey, this.email), this.setCurrentPaymentMethod(i), this.cleanup(), this.renderPaymentMethodContent(), this.attachInputListeners();
        }
      });
  ...
  setCurrentPaymentMethod(A) {
    this.currentPaymentMethod = A;
    const t = document.querySelectorAll(".tab-button"), e = document.querySelectorAll(".content");
    t.forEach((n) => n.classList.remove("active")), e.forEach((n) => n.classList.remove("active")), t[A] && t[A].classList.add("active"), e[A] && e[A].classList.add("active");
  }
  cleanup() {
  ...
  setup() {
    const A = document.getElementById("merchantLogo");
    A && A.setAttribute("src", J);
    let t = document.querySelector(".payment-options");
    t && (t.innerHTML = `<div> 
      ${k.map(
      (s, o) => `
            <div class="tab">   
    ...
    k.forEach((s, o) => {
      const c = e[o];
      if (c) {
        const C = c.querySelector(".tab-button-icon"), l = c.querySelector(".tab-button-label");
        C && (C.innerHTML = s.icon), l && (l.textContent = s.label), c.dataset.tab = o.toString(), c.addEventListener("click", () => {
          this.setCurrentPaymentMethod(o);
        });
      }
    }), this.displayTabLayout();
    const n = this.modalContainer.querySelector("#paymentWarning"), i = n == null ? void 0 : n.querySelector("#payment-warning-text");
    i && this.displayPaymentWarningText(this.currentPaymentMethod, i);
  }
  displayPaymentWarningText(A, t) {
    const e = () => {
      switch (A) {
        case 0:
