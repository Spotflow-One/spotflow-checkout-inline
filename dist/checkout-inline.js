...
const U = `<div role="dialog" class="relative z-10" aria-labelledby="modal-title" aria-modal="true">
...
const E = (r) => {
...
}, g = "http://dev-api.spotflow.one/api/v1", O = async (r, e) => {
  try {
    const t = E(r), A = await fetch(`${g}/payments`, {
      method: "POST",
      headers: t,
    ...
    if (!A.ok) {
      ...
      throw s.message ? new C(s.message, A.status) : new C("Network response was not ok", A.status);
    }
    return await A.json();
  } catch (t) {
    throw t instanceof C ? console.error(`FetchError: ${t.message} (status: ${t.status})`) : t instanceof u ? console.error(`AuthorizationError: ${t.message}`) : console.error(`Unexpected error: ${t}`), t;
  }
}, N = async (r, e) => {
  try {
    const t = E(r), A = await fetch(`${g}/payments/authorize`, {
      method: "POST",
      headers: t,
      body: JSON.stringify(e)
    });
  ...
  try {
    const t = E(r), A = await fetch(`${g}/payments/validate`, {
      method: "POST",
      headers: t,
      body: JSON.stringify(e)
    });
  ...
  try {
    const A = E(r), n = await fetch(`${g}/payments/verify?reference=${e}`, {
      method: "GET",
      headers: A,
      signal: t
    });
    if (!n.ok) {
      ...
      throw i.message ? new C(i.message, n.status) : new C("Network response was not ok", n.status);
    }
    return await n.json();
  } catch (A) {
    throw A instanceof C ? console.error(`FetchError: ${A.message} (status: ${A.status})`) : A instanceof u ? console.error(`AuthorizationError: ${A.message}`) : console.error(`Unexpected error: ${A}`), A;
  }
}, H = () => {
...
class M {
  constructor(e, t, A, n) {
    ...
    a(this, "_currentStep");
    a(this, "activeRef");
    this.container = e, this.closeModal = t, this.token = A, this.email = n, this._currentStep = 1, this.cardDetailsValues = {
      number: "",
      expiry: "",
      cvv: ""
    }, this.cardPin = "", this.cardOtp = "", this.activeRef = "", this.contents = document.querySelectorAll(".content"), this.renderCardContent(), this.attachInputListeners();
  }
  get currentStep() {
  ...
  attachInputListeners() {
    const e = this.container.querySelector(
      ...
      ".otp-button"
    ), h = this.container.querySelector(
      ".success-button"
    );
    e && e.addEventListener(
      "input",
      (l) => this.handleInputChange(l, n)
      ...
      "input",
      (l) => this.handleInputChange(l, n)
    ), s && s.addEventListener("submit", (l) => this.handleSubmit(l)), i.forEach((l, p) => {
      l.addEventListener(
        "input",
        (w) => this.handlePinInputChange(w, p, i)
      ), l.addEventListener(
        "paste",
      ...
      "input",
      (l) => this.handleOtpInput(l, c)
    ), c && c.addEventListener("click", (l) => this.submitOtp(l)), h && h.addEventListener("click", () => this.closeModal());
  }
  handleInputChange(e, t) {
    const A = e.target, { name: n, value: s } = A;
    let i = s;
    n === "number" ? i = P(s) : n === "expiry" ? i = W(s) : n === "cvv" && (i = Z(s)), this.cardDetailsValues = {
      ...this.cardDetailsValues,
      [n]: i
    }, A.value = i, this.cardDetailsValues.number && this.cardDetailsValues.expiry && this.cardDetailsValues.cvv && t && t.removeAttribute("disabled");
  }
  handlePinRequest() {
    const e = this.container.querySelector("#loader"), t = this.container.querySelector("#pin-content");
    if (this.cardPin.length === 4) {
      ...
      setTimeout(() => {
        var s;
        e && t && (e.classList.remove("hidden"), (s = e.querySelector("img")) == null || s.setAttribute("src", y), t.classList.add("hidden")), N(this.token, A).then((i) => {
          i.status === "failed" ? i.providerMessage ? d(i.providerMessage, "error") : d("Payment failed", "error") : (d("Payment Authorized!", "success"), this.currentStep = 3);
        }).catch((i) => {
          d(i.message, "error");
        }).finally(() => {
  ...
  handlePinInputChange(e, t, A) {
    this.cardPin = "", e.target.value ? t < A.length - 1 && A[t + 1].focus() : t > 0 && A[t - 1].focus(), this.cardPin = A.map((i) => i.value).join(""), this.handlePinRequest();
  }
  handlePinPaste(e, t) {
    var s;
    e.preventDefault(), this.cardPin = "";
    const n = (((s = e.clipboardData) == null ? void 0 : s.getData("text").trim()) || "").split("");
    t.forEach((i, o) => {
      i.value = n[o] || "";
    });
  ...
  handleOtpInput(e, t) {
    const A = e.target, { value: n } = A;
    this.cardOtp = "", this.cardOtp = n, this.cardOtp.length > 3 && t && t.removeAttribute("disabled");
  }
  submitOtp(e) {
    var i;
    e.preventDefault();
    const t = this.container.querySelector("#loader"), A = this.container.querySelector("#otp-content"), n = {
      authorization: {
        otp: this.cardPin
      },
      reference: this.activeRef
    };
    t && A && (t.classList.remove("hidden"), (i = t.querySelector("img")) == null || i.setAttribute("src", y), A.classList.add("hidden")), Y(this.token, n).then((o) => {
      o.status === "failed" ? o.providerMessage ? d(o.providerMessage, "error") : d("Payment failed", "error") : o.status === "successful" && (this.currentStep = 5);
    }).catch((o) => {
      d(o.message, "error");
    }).finally(() => {
      t && A && (t.classList.add("hidden"), A.classList.remove("hidden"));
    });
  }
  async handleSubmit(e) {
    var i, o;
    e.preventDefault();
    const t = this.container.querySelector(".spinner"), A = this.container.querySelector(
      ...
      reference: H(),
      card: {
        pan: j(this.cardDetailsValues.number),
        cvv: this.cardDetailsValues.cvv,
        expiryMonth: (i = this.cardDetailsValues) == null ? void 0 : i.expiry.split("/")[0].trim(),
        expiryYear: (o = this.cardDetailsValues) == null ? void 0 : o.expiry.split("/")[1].trim()
      }
    };
    t && A && (t.classList.remove("hidden"), A.classList.add("hidden")), O(this.token, n).then((c) => {
      c.status === "failed" ? c.providerMessage ? d(c.providerMessage, "error") : d("Payment failed", "error") : c.status === "pending_authorization" && (d("Payment Created!", "success"), this.activeRef = c.reference, this.currentStep = 2);
    }).catch((c) => {
      d(c.message, "error");
    }).finally(() => {
      t && A && (t.classList.add("hidden"), A.classList.remove("hidden"));
...
class et {
  constructor(e, t, A, n) {
    ...
    a(this, "_currentStep");
    a(this, "activeRef");
    a(this, "paymentResponse");
    a(this, "initialTime");
    a(this, "timeLeft");
    a(this, "timerId");
    a(this, "abortController", null);
    this.container = e, this.contents = document.querySelectorAll(".content"), this.closeModal = t, this.token = A, this.email = n, this._currentStep = 1, this.renderTransferContent(), this.paymentResponse = null, this.activeRef = "", this.initialTime = 600, this.timeLeft = this.initialTime, this.timerId = null, this.attachInputListeners(), this.currentStep === 1 && (this.abortController = new AbortController(), this.createTransfer());
  }
  get currentStep() {
    return this._currentStep;
  }
  ...
  attachInputListeners() {
    const e = this.container.querySelector("#transfer-acn"), t = this.container.querySelector("#copy-acn"), A = this.container.querySelector("#acn-tooltip"), n = this.container.querySelector("#transfer-amount"), s = this.container.querySelector("#copy-amount"), i = this.container.querySelector("#amount-tooltip"), o = this.container.querySelector("#transfer-try-again"), c = this.container.querySelector("#transfer-sent"), h = this.container.querySelector(".transfer-button"), l = this.container.querySelector("#show-acn"), p = this.container.querySelector("#ctd-timer"), w = this.container.querySelector(
      ".success-button"
    ), S = this.container.querySelector("#confirm-one"), x = this.container.querySelector(".transfer-close");
    if (t && e && t.addEventListener("click", () => {
      A && this.copyToClipboard(e.innerHTML, A);
    }), s && n && s.addEventListener("click", () => {
      i && this.copyToClipboard(n.innerHTML.split(" ")[1], i);
      ...
      this.initialTime = 300, this.timeLeft = this.initialTime, p && (p.innerHTML = Q(this.initialTime));
      const b = 3e4;
      this.abortController = new AbortController(), setTimeout(() => {
        this.verifyTransfer(
          ...
          (v) => {
            console.error("Polling failed:", v), d(v.message, "error");
          }
        );
    ...
    S && S.addEventListener("click", () => {
      this.currentStep = 4;
    }), w && w.addEventListener("click", () => this.closeModal()), x && x.addEventListener("click", () => this.closeModal());
  }
  async createTransfer() {
    requestAnimationFrame(() => {
      var s;
      const e = this.container.querySelector("#loader"), t = this.container.querySelector("#details-content");
      e && t && (e.classList.remove("hidden"), (s = e.querySelector("img")) == null || s.setAttribute("src", y), t.classList.add("hidden"));
      const A = {
        amount: 14.99,
      ...
      z(this.token, A).then((i) => {
        if (i.status === "failed")
          i.providerMessage ? d(i.providerMessage, "error") : d("Payment failed", "error"), this.currentStep = 2;
        else {
          ...
          setTimeout(() => {
            this.verifyTransfer(
              ...
              (c) => {
                console.error("Polling failed:", c), d(c.message, "error");
              }
            );
          }, o);
        }
  ...
  async verifyTransfer(e, t, A, n) {
    let s = 0, i = t, o = !0, c;
    this.abortController && (c = this.abortController.signal);
    const h = async () => {
      if (this.timeLeft <= 0) {
        o = !1, n(new Error("Max polling time reached")), this.currentStep = 2;
        return;
      }
      s += 1;
      try {
        ...
        l.status === "failed" ? o && (i = t * (s + 1), setTimeout(h, i)) : (o = !1, A(l));
      } catch {
        o && (i = t * (s + 1), setTimeout(h, i));
      }
    };
    h();
  }
  ...
  updateTransferDetailsContent() {
    var s;
    const e = this.container.querySelectorAll(".transfer-curr-amount"), t = this.container.querySelector("#transfer-bank"), A = this.container.querySelector("#transfer-acn"), n = this.container.querySelector("#time-left");
    this.paymentResponse && (e.length && e.forEach(
      (i) => {
        var o, c;
        return i.innerHTML = `${(o = this.paymentResponse) == null ? void 0 : o.currency} ${(c = this.paymentResponse) == null ? void 0 : c.amount}`;
      }
    ), t && this.paymentResponse.bankDetails && (t.innerHTML = (s = this.paymentResponse.bankDetails) == null ? void 0 : s.bankName), A && this.paymentResponse.bankDetails && (A.innerHTML = this.paymentResponse.bankDetails.accountNumber), n && (n.innerHTML = Q(this.initialTime)), this.startTimer("poll"));
  }
  ...
  startTimer(e) {
    ...
    this.stopTimer(), this.timerId = window.setInterval(() => {
      this.timeLeft > 0 ? (this.timeLeft -= 1, e === "poll" ? this.updateProgressTimerContent() : t && (t.innerHTML = Q(this.timeLeft), this.startTimer("check"))) : (this.stopTimer(), e === "check" && (this.currentStep = 3));
    }, 1e3);
  }
  updateProgressTimerContent() {
    const t = 2 * Math.PI * 40, n = (this.initialTime - this.timeLeft) / this.initialTime * 100 / 100 * t, s = this.container.querySelector(".circle-progress"), i = this.container.querySelector("#time-left");
    s && (s.setAttribute("stroke-dasharray", `${t}`), s.setAttribute("stroke-dashoffset", `${n}`)), i && (i.innerHTML = Q(this.timeLeft));
  }
  stopTimer() {
...
class st {
  constructor() {
    ...
    a(this, "banks");
    a(this, "_currentStep");
    this.contents = document.querySelectorAll(".content"), this.banks = nt, this._currentStep = 1, this.renderUssdContent();
  }
  get currentStep() {
    return this._currentStep;
  }
  ...
  renderSearchOptions(e, t, A) {
    A.innerHTML = "", e.forEach((n) => {
      const s = document.createElement("div");
      s.classList.add("bank-option"), s.textContent = n.name, s.addEventListener("click", () => {
        t.value = n.name, A.style.display = "none";
      }), A.appendChild(s);
    }), A.style.display = e.length ? "block" : "none";
  }
  searchBanksOnInput(e, t) {
    const A = e.value.toLowerCase(), n = this.banks.filter(
      (s) => s.name.toLowerCase().includes(A)
    );
    this.renderSearchOptions(n, e, t);
  }
  openSearchOnFocus(e, t) {
    this.renderSearchOptions(this.banks, e, t);
  }
  closeSearchOptionsOutsideFocus(e, t, A) {
    !t.contains(e.target) && !A.contains(e.target) && (A.style.display = "none");
  }
}
class rt {
  constructor(e, t) {
    a(this, "modalContainer");
    a(this, "currentPaymentMethod");
    a(this, "currentStep");
    a(this, "container");
    a(this, "card");
    a(this, "transfer");
    ...
    a(this, "merchantKey");
    a(this, "email");
    this.modalContainer = document.createElement("div"), this.modalContainer.innerHTML = U, document.body.appendChild(this.modalContainer), this.currentPaymentMethod = 0, this.currentStep = 1, this.container = document.querySelectorAll(".content")[0], this.merchantKey = e, this.email = t, this.updatePaymentMethodView(this.merchantKey, this.email), this.attachInputListeners();
  }
  attachInputListeners() {
    const e = this.modalContainer.querySelector(
      ".success-button"
    ), t = document.body.querySelector(
      "#close-modal"
    );
    e && (this.cleanup(), e.addEventListener("click", this.closeModal)), t && (this.cleanup(), t.addEventListener("click", () => this.closeModal()));
  }
  displayTabLayout() {
    const e = document.querySelectorAll(".tab-button"), t = document.querySelector(".box-container-method");
    e && t && e.forEach((A) => {
      A.addEventListener("click", (n) => {
        if (n.currentTarget instanceof HTMLElement && n.currentTarget.dataset.tab) {
          const s = parseInt(n.currentTarget.dataset.tab);
          this.updatePaymentMethodView(this.merchantKey, this.email), this.setCurrentPaymentMethod(s), this.cleanup(), this.renderPaymentMethodContent(), this.attachInputListeners();
        }
      });
    }), this.updatePaymentMethodView(this.merchantKey, this.email), this.renderPaymentMethodContent(), this.attachInputListeners(), this.setCurrentPaymentMethod(this.currentPaymentMethod);
  }
  updatePaymentMethodView(e, t) {
    switch (this.currentPaymentMethod) {
      case 0:
        return this.card = new M(
          this.modalContainer,
          () => this.closeModal(),
          e,
          t
  ...
  setCurrentPaymentMethod(e) {
    ...
    const t = document.querySelectorAll(".tab-button"), A = document.querySelectorAll(".content");
    t.forEach((n) => n.classList.remove("active")), A.forEach((n) => n.classList.remove("active")), t[e] && t[e].classList.add("active"), A[e] && A[e].classList.add("active");
  }
  cleanup() {
  ...
  setup() {
    const e = document.getElementById("merchantLogo"), t = document.getElementById("merchant-email");
    e && e.setAttribute("src", J), t && (t.innerHTML = this.email);
    let A = document.querySelector(".payment-options");
    A && (A.innerHTML = `<div> 
      ${k.map(
      (o, c) => `
            <div class="tab">   
    ...
    k.forEach((o, c) => {
      const h = n[c];
      if (h) {
        const l = h.querySelector(".tab-button-icon"), p = h.querySelector(".tab-button-label");
        l && (l.innerHTML = o.icon), p && (p.textContent = o.label), h.dataset.tab = c.toString(), h.addEventListener("click", () => {
          this.setCurrentPaymentMethod(c);
        });
      }
    }), this.displayTabLayout();
    const s = this.modalContainer.querySelector("#paymentWarning"), i = s == null ? void 0 : s.querySelector("#payment-warning-text");
    i && this.displayPaymentWarningText(this.currentPaymentMethod, i);
  }
  displayPaymentWarningText(e, t) {
    const A = () => {
      switch (e) {
        case 0:
