...
const K = `<div role="dialog" class="relative z-10" aria-labelledby="modal-title" aria-modal="true">
...
const L = (r) => {
  if (!r)
    throw new m("Missing authorization token");
  const e = new Headers();
  return e.append("Content-Type", "application/json"), e.append("Authorization", `Bearer ${r}`), e;
}, Q = "http://dev-api.spotflow.one/api/v1", q = async (r, e) => {
  try {
    const t = L(r), n = await fetch(`${Q}/payments`, {
      method: "POST",
      headers: t,
    ...
    if (!n.ok) {
      ...
      throw s.message ? new u(s.message, n.status) : new u("Network response was not ok", n.status);
    }
    return await n.json();
  } catch (t) {
    throw t instanceof u ? console.error(`FetchError: ${t.message} (status: ${t.status})`) : t instanceof m ? console.error(`AuthorizationError: ${t.message}`) : console.error(`Unexpected error: ${t}`), t;
  }
}, Y = async (r, e) => {
  try {
    const t = L(r), n = await fetch(`${Q}/payments/authorize`, {
      method: "POST",
      headers: t,
      body: JSON.stringify(e)
    });
  ...
  try {
    const t = L(r), n = await fetch(`${Q}/payments/validate`, {
      method: "POST",
      headers: t,
      body: JSON.stringify(e)
    });
  ...
  try {
    const n = L(r), A = await fetch(`${Q}/payments/verify?reference=${e}`, {
      method: "GET",
      headers: n,
      signal: t
    });
    if (!A.ok) {
      ...
      throw i.message ? new u(i.message, A.status) : new u("Network response was not ok", A.status);
    }
    return await A.json();
  } catch (n) {
    throw n instanceof u ? console.error(`FetchError: ${n.message} (status: ${n.status})`) : n instanceof m ? console.error(`AuthorizationError: ${n.message}`) : console.error(`Unexpected error: ${n}`), n;
  }
}, T = () => {
...
class R {
  constructor(e, t, n, A, s) {
    ...
    this.container = e, this.closeModal = t, this.token = n, this.email = A, this.amount = s, this._currentStep = 1, this.cardDetailsValues = {
      ...
      expiry: "",
      cvv: ""
    }, this.cardPin = "", this.cardOtp = "", this.activeRef = "", this.contents = document.querySelectorAll(".content"), this.creditCardTypes = [...y], this.renderCardContent(), this.currentStep === 1 && this.displayCardTypes(), this.attachInputListeners();
  }
  displayCardTypes() {
    requestAnimationFrame(() => {
      ...
      this.creditCardTypes.length === 1 && this.creditCardTypes[0].name === "Unknown" ? y.forEach((t) => {
  ...
  set currentStep(e) {
    this._currentStep = e, this.renderCardContent(), e === 1 && this.displayCardTypes();
  }
  attachInputListeners() {
    ...
    e && e.addEventListener(
      ...
      "input",
      (l) => this.handleInputChange(l, A)
    ), s && s.addEventListener("submit", (l) => this.handleSubmit(l)), i.forEach((l, d) => {
      l.addEventListener(
        "input",
        (p) => this.handlePinInputChange(p, d, i)
      ), l.addEventListener(
        "paste",
        (p) => this.handlePinPaste(p, i)
      );
    }), c && c.addEventListener(
      "input",
      (l) => this.handleOtpInput(l, a)
    ), a && a.addEventListener("click", (l) => this.submitOtp(l)), C && C.addEventListener("click", () => this.closeModal());
  }
  handleInputChange(e, t) {
    const n = e.target, { name: A, value: s } = n;
    let i = s;
    A === "number" ? (i = P(s), this.filterCreditCardType(s)) : A === "expiry" ? i = j(s) : A === "cvv" && (i = W(s)), this.cardDetailsValues = {
  ...
  handlePinRequest() {
    ...
    if (this.cardPin.length === 4) {
      ...
      setTimeout(() => {
        var s;
        e && t && (e.classList.remove("hidden"), (s = e.querySelector("img")) == null || s.setAttribute("src", H), t.classList.add("hidden")), Y(this.token, n).then((i) => {
          i.status === "failed" ? i.providerMessage ? h(i.providerMessage, "error") : h("Payment failed", "error") : (h("Payment Authorized!", "success"), this.currentStep = 3);
        }).catch((i) => {
          h(i.message, "error");
        }).finally(() => {
  ...
  showLoader() {
    ...
    e && (e.classList.remove("hidden"), (t = e.querySelector("img")) == null || t.setAttribute("src", H));
  }
  handlePinPaste(e, t) {
    var s;
  ...
  submitOtp(e) {
    var i;
    e.preventDefault();
    const t = this.container.querySelector("#loader"), n = this.container.querySelector("#otp-content"), A = {
      authorization: {
        otp: this.cardPin
      },
      reference: this.activeRef
    };
    t && n && (t.classList.remove("hidden"), (i = t.querySelector("img")) == null || i.setAttribute("src", H), n.classList.add("hidden")), z(this.token, A).then((c) => {
      c.status === "failed" ? c.providerMessage ? h(c.providerMessage, "error") : h("Payment failed", "error") : c.status === "successful" && (this.currentStep = 5);
    }).catch((c) => {
      h(c.message, "error");
    }).finally(() => {
  ...
  filterCreditCardType(e) {
    e ? (this.creditCardTypes = y.filter(
      (t) => t.name === $(e)
    ), this.creditCardTypes.length >= 1 ? this.displayCardTypes() : (this.creditCardTypes = [...y], this.displayCardTypes())) : (this.creditCardTypes = [...y], this.displayCardTypes());
  }
  renderCardContent() {
...
class i1 {
  ...
  attachInputListeners() {
    ...
    if (t && e && t.addEventListener("click", () => {
      ...
      this.initialTime = 300, this.timeLeft = this.initialTime, d && (d.innerHTML = B(this.initialTime));
      const U = 3e4;
      this.abortController = new AbortController(), setTimeout(() => {
        this.verifyTransfer(
          ...
          (M) => {
            console.error("Polling failed:", M), h(M.message, "error");
          }
        );
    ...
    w && w.addEventListener("click", () => {
      this.currentStep = 4;
    }), p && p.addEventListener("click", () => this.closeModal()), g && g.addEventListener("click", () => this.closeModal());
  }
  async createTransfer() {
    requestAnimationFrame(() => {
      var s;
      const e = this.container.querySelector("#loader"), t = this.container.querySelector("#details-content");
      e && t && (e.classList.remove("hidden"), (s = e.querySelector("img")) == null || s.setAttribute("src", H), t.classList.add("hidden"));
      const n = {
        amount: this.amount,
  ...
  async verifyTransfer(e, t, n, A) {
    ...
    const C = async () => {
      ...
      try {
        ...
        l.status === "failed" ? c && (i = t * (s + 1), setTimeout(C, i)) : (c = !1, n(l));
      } catch {
        c && (i = t * (s + 1), setTimeout(C, i));
      }
    };
    C();
  }
...
class c1 {
  constructor(e, t, n) {
    ...
    o(this, "amount");
    o(this, "isMobile", !1);
    if (this.modalContainer = document.createElement("div"), this.modalContainer.innerHTML = K, document.body.appendChild(this.modalContainer), this.isMobile = window.matchMedia("(max-width: 767px)").matches, this.currentPaymentMethod = this.isMobile ? 3 : 0, this.currentStep = 1, this.container = document.querySelectorAll(".content")[0], this.merchantKey = e, this.email = t, this.amount = n, this.attachInputListeners(), e === "" || t === "")
    ...
    this.isMobile ? t && n && A && s && t.forEach((c) => {
      c.addEventListener("click", (a) => {
        if (a.currentTarget instanceof HTMLElement && a.currentTarget.dataset.tab) {
          const C = parseInt(a.currentTarget.dataset.tab);
          this.updatePaymentMethodView(this.merchantKey, this.email, this.amount), this.setCurrentPaymentMethod(C), A.removeChild(s), this.cleanup(), E.forEach((l, d) => {
            this.currentPaymentMethod === d && i && (i.innerHTML = `
      ...
      c.addEventListener("click", (a) => {
        if (a.currentTarget instanceof HTMLElement && a.currentTarget.dataset.tab) {
          const C = parseInt(a.currentTarget.dataset.tab);
          this.updatePaymentMethodView(this.merchantKey, this.email, this.amount), this.setCurrentPaymentMethod(C), this.cleanup(), this.renderPaymentMethodContent(), this.attachInputListeners();
        }
      });
  ...
  updatePaymentMethodView(e, t, n) {
    switch (this.currentPaymentMethod) {
      case 0:
        return this.card = new R(this.modalContainer, () => this.closeModal(), e, t, n);
      case 1:
        return this.transfer = new i1(this.modalContainer, () => this.closeModal(), e, t, n);
      case 2:
  ...
  setup() {
    const e = document.getElementById("merchantLogo"), t = document.getElementById("merchantLogoMob"), n = document.getElementById("mob-action-btns"), A = document.getElementById("merchant-email"), s = this.modalContainer.querySelector("#paymentWarning"), i = s == null ? void 0 : s.querySelector("#payment-warning-text"), c = document.querySelector("#mob-container");
    if (A && (A.innerHTML = this.email), this.isMobile) {
      t && t.setAttribute("src", x), c && (c.innerHTML = this.mobileContainerContent());
      let a = document.querySelector(".mob-options");
      a && this.viewMobileOptions(a);
      const C = document.querySelectorAll(".mob-tab-button");
      E.forEach((l, d) => {
        const p = C[d];
        p && (p.dataset.tab = d.toString(), p.addEventListener("click", () => {
          this.setCurrentPaymentMethod(d);
        }));
      }), n && this.currentPaymentMethod === 3 && (n.classList.add("hidden"), n.classList.remove("md:hidden"));
    } else {
      e && e.setAttribute("src", x);
      ...
      E.forEach((l, d) => {
        const p = C[d];
        if (p) {
          const w = p.querySelector(".tab-button-icon"), g = p.querySelector(".tab-button-label");
          w && (w.innerHTML = l.icon), g && (g.textContent = l.label), p.dataset.tab = d.toString(), p.addEventListener("click", () => {
            this.setCurrentPaymentMethod(d);
          });
        }
      });
    }
    this.displayTabLayout(), i && this.displayPaymentWarningText(this.currentPaymentMethod, i);
  }
