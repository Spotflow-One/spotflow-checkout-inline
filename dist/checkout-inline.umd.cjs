...
(function(w,C){typeof exports=="object"&&typeof module<"u"?module.exports=C():typeof define=="function"&&define.amd?define(C):(w=typeof globalThis<"u"?globalThis:w||self,w.SpotflowCheckout=C())})(this,function(){"use strict";var st=Object.defineProperty;var it=(w,C,u)=>C in w?st(w,C,{enumerable:!0,configurable:!0,writable:!0,value:u}):w[C]=u;var a=(w,C,u)=>it(w,typeof C!="symbol"?C+"":C,u);const w=`<div role="dialog" class="relative z-10" aria-labelledby="modal-title" aria-modal="true">
...
`;class ${constructor(e,t,A,n){a(this,"container");a(this,"contents");a(this,"closeModal");a(this,"email");a(this,"token");a(this,"_currentStep");a(this,"activeRef");a(this,"paymentResponse");a(this,"initialTime");a(this,"timeLeft");a(this,"timerId");a(this,"abortController",null);this.container=e,this.contents=document.querySelectorAll(".content"),this.closeModal=t,this.token=A,this.email=n,this._currentStep=1,this.renderTransferContent(),this.paymentResponse=null,this.activeRef="",this.initialTime=600,this.timeLeft=this.initialTime,this.timerId=null,this.attachInputListeners(),this.currentStep===1&&(this.abortController=new AbortController,this.createTransfer())}get currentStep(){return this._currentStep}set currentStep(e){this._currentStep=e,this.renderTransferContent(),e===1&&this.createTransfer(),e===5&&this.startTimer("check")}attachInputListeners(){const e=this.container.querySelector("#transfer-acn"),t=this.container.querySelector("#copy-acn"),A=this.container.querySelector("#acn-tooltip"),n=this.container.querySelector("#transfer-amount"),i=this.container.querySelector("#copy-amount"),s=this.container.querySelector("#amount-tooltip"),o=this.container.querySelector("#transfer-try-again"),c=this.container.querySelector("#transfer-sent"),p=this.container.querySelector(".transfer-button"),l=this.container.querySelector("#show-acn"),S=this.container.querySelector("#ctd-timer"),g=this.container.querySelector(".success-button"),F=this.container.querySelector("#confirm-one"),b=this.container.querySelector(".transfer-close");if(t&&e&&t.addEventListener("click",()=>{A&&this.copyToClipboard(e.innerHTML,A)}),i&&n&&i.addEventListener("click",()=>{s&&this.copyToClipboard(n.innerHTML.split(" ")[1],s)}),p&&p.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),o&&o.addEventListener("click",()=>{this.currentStep=1}),c&&c.addEventListener("click",()=>{this.abortController&&(this.abortController.abort(),this.currentStep=5)}),l&&l.addEventListener("click",()=>{this.stopProgressBar(),this.currentStep=1}),this.currentStep===5){this.initialTime=300,this.timeLeft=this.initialTime,S&&(S.innerHTML=B(this.initialTime));const nt=3e4;this.abortController=new AbortController,setTimeout(()=>{this.verifyTransfer(this.token,nt,()=>{this.currentStep=6},T=>{console.error("Polling failed:",T),d(T.message,"error")})},5e3),this.startTimer("check")}F&&F.addEventListener("click",()=>{this.currentStep=4}),g&&g.addEventListener("click",()=>this.closeModal()),b&&b.addEventListener("click",()=>this.closeModal())}async createTransfer(){requestAnimationFrame(()=>{var i;const e=this.container.querySelector("#loader"),t=this.container.querySelector("#details-content");e&&t&&(e.classList.remove("hidden"),(i=e.querySelector("img"))==null||i.setAttribute("src",v),t.classList.add("hidden"));const A={amount:14.99,channel:"bank_transfer",currency:"USD",customer:{email:this.email},reference:M()};O(this.token,A).then(s=>{if(s.status==="failed")s.providerMessage?d(s.providerMessage,"error"):d("Payment failed","error"),this.currentStep=2;else{e&&t&&(e.classList.add("hidden"),t.classList.remove("hidden")),this.activeRef=s.reference,this.paymentResponse=s,this.updateTransferDetailsContent();const o=3e4;setTimeout(()=>{this.verifyTransfer(this.token,o,()=>{this.currentStep=6},c=>{console.error("Polling failed:",c),d(c.message,"error")})},o)}}).catch(s=>{d(s.message,"error")})})}async verifyTransfer(e,t,A,n){let i=0,s=t,o=!0,c;this.abortController&&(c=this.abortController.signal);const p=async()=>{if(this.timeLeft<=0){o=!1,n(new Error("Max polling time reached")),this.currentStep=2;return}i+=1;try{const l=await N(e,this.activeRef,c);l.status==="failed"?o&&(s=t*(i+1),setTimeout(p,s)):(o=!1,A(l))}catch{o&&(s=t*(i+1),setTimeout(p,s))}};p()}getTransferStepContent(){switch(this.currentStep){case 1:return k;case 2:return W;case 3:return j;case 4:return V;case 5:return X;case 6:return x;default:return k}}updateTransferDetailsContent(){var i;const e=this.container.querySelectorAll(".transfer-curr-amount"),t=this.container.querySelector("#transfer-bank"),A=this.container.querySelector("#transfer-acn"),n=this.container.querySelector("#time-left");this.paymentResponse&&(e.length&&e.forEach(s=>{var o,c;return s.innerHTML=`${(o=this.paymentResponse)==null?void 0:o.currency} ${(c=this.paymentResponse)==null?void 0:c.amount}`}),t&&this.paymentResponse.bankDetails&&(t.innerHTML=(i=this.paymentResponse.bankDetails)==null?void 0:i.bankName),A&&this.paymentResponse.bankDetails&&(A.innerHTML=this.paymentResponse.bankDetails.accountNumber),n&&(n.innerHTML=B(this.initialTime)),this.startTimer("poll"))}renderTransferContent(){this.contents[1]&&(this.contents[1].innerHTML=this.getTransferStepContent(),this.attachInputListeners())}startTimer(e){const t=this.container.querySelector("#ctd-timer");this.stopTimer(),this.timerId=window.setInterval(()=>{this.timeLeft>0?(this.timeLeft-=1,e==="poll"?this.updateProgressTimerContent():t&&(t.innerHTML=B(this.timeLeft),this.startTimer("check"))):(this.stopTimer(),e==="check"&&(this.currentStep=3))},1e3)}updateProgressTimerContent(){const t=2*Math.PI*40,n=(this.initialTime-this.timeLeft)/this.initialTime*100/100*t,i=this.container.querySelector(".circle-progress"),s=this.container.querySelector("#time-left");i&&(i.setAttribute("stroke-dasharray",`${t}`),i.setAttribute("stroke-dashoffset",`${n}`)),s&&(s.innerHTML=B(this.timeLeft))}stopTimer(){this.timerId!==null&&(clearInterval(this.timerId),this.timerId=null)}stopProgressBar(){const e=this.container.querySelector(".progress-bar");e&&(e.style.animationPlayState="paused")}copyToClipboard(e,t){navigator.clipboard.writeText(e).then(()=>{t.innerHTML="Copied!"}).catch(()=>{t.innerHTML="Failed to copy"})}destroyTimer(){this.stopTimer()}stopPolling(){this.abortController&&this.abortController.abort()}}const L=`<div id="ussdBanks">
  <div class="box-container">
